...
class Storage {
  ...
  static void renumber(TypeData type) {
    for (var i = 0; i < type.references.length; i++) {
      type.references[i] = type.references[i].copyWith(
        referenceNumber: i + 1,
      );
    }
  }
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> load() async {
    ...
    if (mounted) {
      setState(() => groupCount = groups.length);
    }
  }
...
class _CreateGroupPageState extends State<CreateGroupPage> {
  ...
  Future<void> create() async {
    ...
    // สร้างกลุ่มเสร็จแล้วกลับหน้าแรก
...
class _GroupListPageState extends State<GroupListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: groups.isEmpty
          ? const Center(
              child: Text('ยังไม่มีรายการที่บันทึก'),
            )
          : ListView.builder(
          ...
          : t.references.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final scannedAreas = ref.scans.length;
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                          'มีข้อมูล $scannedAreas/${areas.length} ด้าน',
                        ),
                        trailing: PopupMenuButton<String>(
                          onSelected: (value) {
                            if (value == 'delete') {
      ...
      for (final m in g.models) {
        ...
        for (final t in m.types) {
          ...
          for (var i = 0; i < t.references.length; i++) {
            ...
            if (mounted) {
              setState(() {
                scans[selectedArea] = result;
              });
            }

            showMessage(
              'บันทึกข้อมูล $selectedArea แล้ว',
            );
            return;
          }
        }
