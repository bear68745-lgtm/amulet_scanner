  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: group!.models.isEmpty
          ? const Center(child: Text('ยังไม่มีข้อมูลรุ่น'))
          : ListView.builder(
              padding: const EdgeInsets.all(12),
...
class _CreateDataPageState extends State<CreateDataPage> {
  ...
  Future<void> save() async {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('กรุณากรอกชื่อสำนัก/รุ่น'),
        ),
      );
      return;
    }
      ...
      body: ListView(
        ...
        children: [
          TextField(
            controller: nameController,
            decoration: const InputDecoration(
              labelText: 'ชื่อสำนัก/รุ่น *',
              hintText: 'เช่น หลวงปู่ทวด รุ่นแรก',
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => ScanPage(
          ...
          referenceId: null,
          referenceNumber: type!.references.length + 1,
        ),
      ),
    );
...
class _ScanPageState extends State<ScanPage> {
  ...
  Future<void> saveScan({
    ...
    if (!fromGallery && tempPath != null && tempPath.isNotEmpty) {
      try {
