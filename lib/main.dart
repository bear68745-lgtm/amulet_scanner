...
class Storage {
  ...
  static Iterable<_RefLink> links(
    GroupData g,
  ) sync* {
...
class _CreateReferencePageState
  ...
  // -----------------------------------------------------
  // ชนิดที่ระบบกำหนดพิมพ์ให้อัตโนมัติ
  // -----------------------------------------------------

  bool get isSinglePrintType =>
      selectedType == 'เหรียญ';
  ...
  Future<void> create() async {
    ...
    // ถ้าเป็นเหรียญ ใช้ "พิมพ์นิยม" อัตโนมัติ
    final printName = selectedType == 'เหรียญ'
        ? 'พิมพ์นิยม'
...
class _ReferencePageState
  ...
  Future<int> nextNumber() async {
    ...
    for (final g in gs) {
      for (final x in Storage.links(g)) {
        if (x.reference.referenceNumber >
            max) {
          max = x.reference.referenceNumber;
        }
      }
