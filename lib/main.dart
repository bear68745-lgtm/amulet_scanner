...
class ReferenceData{
  ...
  ReferenceData({
    ...
    DateTime? updatedAt,List<ScanResult>? scans})
  ...
  ReferenceData copyWith({int? referenceNumber,List<ScanResult>? scans})=>
    ReferenceData(
      ...
      createdAt:createdAt,updatedAt:DateTime.now(),scans:scans??this.scans);
}
...
class Storage{
  ...
  static Future<void> addAi({
    ...
    final k=AiKnowledge(
      ...
      groupId:group.id,modelId:model.id,typeId:type.id,printId:print.id,
      referenceId:reference.id,area:scan.area,content:text,
  ...
  static Future<void> correctAi({
    required AiKnowledge old,required String content})async{
    final text=content.trim();
    if(text.isEmpty)return;

    final list=await loadAi(),i=list.indexWhere((e)=>e.id==old.id);
    if(i<0)return;

    final now=DateTime.now();
    list[i]=AiKnowledge(
      id:old.id,groupId:old.groupId,modelId:old.modelId,typeId:old.typeId,
      printId:old.printId,referenceId:old.referenceId,area:old.area,
      content:text,createdAt:old.createdAt,updatedAt:now);
    await saveAi(list);

    final h=await loadHistory();
    h.add(AiLearningHistory(
      id:newId(),groupId:old.groupId,modelId:old.modelId,typeId:old.typeId,
      printId:old.printId,referenceId:old.referenceId,area:old.area,
      content:text,learnedAt:now));
    await saveHistory(h);
  }
  ...
  static Future<void> deleteAiBy({
    ...
    String? referenceId,String? area})async{
      ...
      child:Column(children:[
        SizedBox(width:double.infinity,
          child:FilledButton.icon(
            onPressed:createGroup,icon:const Icon(Icons.add),
...
class _AiMemoryTestPageState extends State<AiMemoryTestPage>{
  ...
  void selectKnowledge(AiKnowledge k)=>setState((){
  ...
  Future<void> saveAnswer()async{
    if(selected==null||answer.text.trim().isEmpty){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content:Text('กรุณาใส่คำตอบก่อน')));return;
    }

    final t=AiMemoryTest(
      id:newId(),knowledgeId:selected!.id,referenceId:selected!.referenceId,
      area:selected!.area,question:question!,answer:answer.text.trim(),
  ...
  Future<void> autoCheck()async{
    ...
    final a=clean(test!.answer),ref=clean(selected!.content);
    String r,why;
    ...
    if(a.isEmpty){
      ...
      final rw=ref.split(RegExp(r'[ ,./]+'))
        .where((e)=>e.isNotEmpty).toSet();
      final score=rw.isEmpty?0:aw.intersection(rw).length/rw.length;
      if(score>=.5){
      ...
      builder:(_)=>AlertDialog(
        ...
        actions:[
          for(final x in ['ถูก','ใกล้เคียง','ผิด','ไม่มีข้อมูล'])
            TextButton(onPressed:()=>Navigator.pop(context,x),child:Text(x))
        ]));
    if(r!=null)await saveResult('ผู้ใช้ตรวจ',r,'ผู้ใช้เป็นผู้ยืนยันผล');
  }
  ...
  Future<void> saveResult(String m,String r,String why)async{
    await Storage.addResult(AiTestResult(
      ...
      referenceId:selected!.referenceId,area:selected!.area,mode:m,
      result:r,answer:test!.answer,referenceAnswer:selected!.content,
      reason:why,checkedAt:DateTime.now()));
  ...
  Future<void> correctKnowledge()async{
      ...
      builder:(_)=>AlertDialog(
        ...
        content:TextField(
          ...
          decoration:const InputDecoration(
            border:OutlineInputBorder(),labelText:'ข้อมูลที่ถูกต้อง',
    ...
    setState((){
      selected=k;result=null;mode=null;
      reason='แก้ไขความรู้แล้ว AI เรียนรู้ข้อมูลใหม่';
      test=null;answer.clear();
    });
  ...
  Widget build(BuildContext context)=>Scaffold(
    ...
    body:knowledge.isEmpty
      ...
      :ListView(
        padding:const EdgeInsets.all(16),
        children:[
          const Text('ขั้นที่ 3–6 : ทดสอบ แก้ไข และเรียนรู้ใหม่',
          ...
          Row(children:[
            Expanded(child:FilledButton.icon(
              onPressed:randomQuestion,icon:const Icon(Icons.shuffle),
            ...
            Expanded(child:OutlinedButton.icon(
              ...
              icon:const Icon(Icons.list),label:const Text('เลือกคำถาม')))
          ]),
          if(selected!=null)...[
            ...
            const SizedBox(height:12),
            TextField(controller:answer,maxLines:6,
              decoration:const InputDecoration(
                labelText:'คำตอบ AI',
            ...
            if(test==null)
              SizedBox(width:double.infinity,
                child:FilledButton.icon(
                  onPressed:saveAnswer,icon:const Icon(Icons.save),
                  label:const Text('บันทึกคำตอบ'))),
            if(test!=null)...[
              const SizedBox(height:8),
              const Text('ขั้นที่ 4 : ตรวจสอบคำตอบ',
                style:TextStyle(fontSize:18,fontWeight:FontWeight.bold)),
              const SizedBox(height:8),
              Row(children:[
                Expanded(child:FilledButton.icon(
                  onPressed:manualCheck,icon:const Icon(Icons.person),
                  label:const Text('ผู้ใช้ตรวจ'))),
                const SizedBox(width:10),
                Expanded(child:OutlinedButton.icon(
                  onPressed:autoCheck,icon:const Icon(Icons.psychology),
                  label:const Text('ตรวจอัตโนมัติ')))
              ]),
              const SizedBox(height:12),
              if(result!=null)Card(child:Padding(
                padding:const EdgeInsets.all(16),
                child:Column(
                  crossAxisAlignment:CrossAxisAlignment.start,children:[
                  Text('ผลตรวจ: $result',style:TextStyle(
                  ...
                  Text('ข้อมูลอ้างอิง:\n${selected!.content}'),
                  if(reason!=null)...[
                    const SizedBox(height:8),Text('เหตุผล: $reason')],
                  if(result=='ผิด'||result=='ใกล้เคียง')...[
                    ...
                    SizedBox(width:double.infinity,
                      child:FilledButton.icon(
                        ...
                        label:const Text(
                          'แก้ไขความรู้และให้ AI เรียนรู้ใหม่')))
                  ]
                ])))
            ]
          ]
      ...
      child:ListView.builder(
        ...
        itemBuilder:(_,i){
          ...
          return Card(child:ListTile(
            ...
            title:Text(k.area),
            subtitle:Text('องค์อ้างอิง ID: ${k.referenceId}',
        ...
        itemBuilder:(_,i){
          ...
          return Card(child:ListTile(
            ...
            onTap:()async{
              await Navigator.push(context,MaterialPageRoute(
                builder:(_)=>TypePage(
                  widget.group,widget.model,t,widget.groups,widget.cameras)));
              setState((){});
            }));
        }));
}
...
class _TypePageState extends State<TypePage>{
  ...
  Future<void> addPrint()async{
    final n=await textDialog(context,'ชื่อพิมพ์',hint:'เว้นว่างได้');
    if(n==null)return;
    widget.type.prints.add(PrintData(
      id:newId(),name:n.trim().isEmpty?'พิมพ์เดียว':n.trim(),
...
class _PrintPageState extends State<PrintPage>{
  ...
  Future<void> deleteReference(ReferenceData r)async{
    if(!await confirm(context,'ลบองค์อ้างอิงที่ ${r.referenceNumber} ?'))return;
...
class _ReferencePageState extends State<ReferencePage>{
  ...
  Future<void> scan(String area)async{
    ...
    await Storage.deleteAiBy(referenceId:widget.reference.id,area:area);
    await Storage.addAi(
      ...
      children:[
        for(final area in areas)
          Card(child:ListTile(
            title:Text(area),
            subtitle:Text(
              getScan(area)==null?'ยังไม่ได้ระบุข้อมูล':
...
class ScanPage extends StatelessWidget{
  ...
  Future<void> enterKnowledge(BuildContext context)async{
      ...
      builder:(_)=>AlertDialog(
        ...
        content:TextField(
          ...
          decoration:const InputDecoration(
            ...
            hintText:'บันทึกลักษณะที่มองเห็นหรือข้อมูลอ้างอิง '
    ...
    Navigator.pop(context,ScanResult(area:area,details:text.trim()));
  }
        ...
        return Stack(
          ...
          children:[
            ...
            Positioned(
              ...
              child:Center(child:FloatingActionButton.large(
                onPressed:capture,child:const Icon(Icons.camera))))
          ]);
      }));
}
          ...
          Padding(
            ...
            child:TextField(
              ...
              decoration:InputDecoration(
                labelText:f,border:const OutlineInputBorder())))
      ])),
    actions:[
      TextButton(onPressed:()=>Navigator.pop(context),
        child:const Text('ยกเลิก')),
...
class _TypeDialogState extends State<TypeDialog>{
  ...
  @override void dispose(){other.dispose();super.dispose();}
