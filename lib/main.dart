...
class _TypePageState extends State<TypePage>{
  ...
  Future<void> addPrint()async{
    ...
    final printName=name.trim().isEmpty?'พิมพ์เดียว':name.trim();
    widget.type.prints.add(PrintData(
      id:newId(),name:printName,createdAt:DateTime.now()));
    await save();setState((){});
  }
  ...
  Future<void> addReference()async{
    ...
    await Navigator.push(context,MaterialPageRoute(
      builder:(_)=>ReferencePage(
        ...
        widget.print,r,widget.groups,widget.cameras)));
  ...
  Widget build(BuildContext context)=>Scaffold(
    ...
    body:ListView(
      ...
      children:[
        ...
        const Text(
          'ขั้นที่ 1: ฐานความรู้ AI\n'
          'ระบบเก็บข้อมูลวิเคราะห์และผูกกับ ID ขององค์อ้างอิง '
...
class ScanPage extends StatelessWidget{
  ...
  const ScanPage({
    required this.area,required this.cameras,super.key});
  ...
  Future<void> scanCamera(BuildContext context)async{
    ...
    final file=await Navigator.push<XFile>(
      context,
      MaterialPageRoute(builder:(_)=>CameraScanPage(camera)));
