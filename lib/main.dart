    ...
    ListView.builder(
      ...
      itemBuilder:(_,i){
        ...
        final refs=g.models.fold<int>(0,(s,m)=>s+m.types.fold<int>(
          0,(tc,t)=>tc+t.prints.fold<int>(
            0,(pc,p)=>pc+p.references.length)));
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(title:Text(g?.name??'รายการ'),actions:[
        FilledButton.icon(
          onPressed:addModel,icon:const Icon(Icons.add,size:22),
      ...
      ListView.builder(
        ...
        itemBuilder:(_,i){
          ...
          return Card(child:ListTile(
            ...
            subtitle:Text('พิมพ์ ${type.prints.length} รายการ'),
            trailing:Row(mainAxisSize:MainAxisSize.min,children:[
...
class _ReferenceListPageState extends State<ReferenceListPage>{
  ...
  Future<void>addReference()async{
    final p=print;if(p==null)return;
          ...
          for(final p in t.prints){
            ...
            for(final ref in p.references){
              if(ref.id==widget.referenceId&&mounted){
                setState(()=>scans={for(final s in ref.scans)s.area:s});
                return;
              }
            }
          }
        ...
        for(final t in m.types){
          ...
          for(final p in t.prints){
            ...
            for(var i=0;i<p.references.length;i++){
              ...
              p.references[i]=ref.copyWith(
                updatedAt:Storage.now(),
                scans:[
                  ...ref.scans.where((e)=>e.area!=selectedArea),result]);
              await Storage.save(gs);
              if(mounted){
      ...
      Card(child:Padding(padding:const EdgeInsets.all(16),
        child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          ...
          ...areas.map((area)=>ListTile(
            dense:true,
            leading:Icon(scans.containsKey(area)
