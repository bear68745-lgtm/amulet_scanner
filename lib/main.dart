...
Future<void> saveAllItems(List<AmuletData> items) async {
  final prefs = await SharedPreferences.getInstance();
  final jsonText = jsonEncode(items.map((e) => e.toMap()).toList());
  final bytes = gzip.encode(utf8.encode(jsonText));
  ...
  Widget build(BuildContext context) {
    final buttons = [
      [
        ...
        Colors.blue,
        () => open(context, CreateDataPage(cameras: cameras)),
      ],
      [
        Icons.list_alt,
        ...
        Colors.green,
        () => open(context, SavedListPage(cameras: cameras)),
      ],
      [
        Icons.import_export,
        'สำรอง / นำเข้าข้อมูล',
        ...
        Colors.orange,
        () => open(context, BackupPage(cameras: cameras)),
      ],
      [
        Icons.settings,
        'ตั้งค่า',
        ...
        Colors.grey,
        () => open(context, SettingsPage(cameras: cameras)),
      ],
    ];
...
class _CreateDataPageState extends State<CreateDataPage> {
  ...
  Future<void> save() async {
    try {
      ...
      if (mounted) message(context, 'บันทึกข้อมูลไม่สำเร็จ: $e');
    }
  }
...
class _EditDataPageState extends State<EditDataPage> {
  ...
  Future<void> save() async {
    try {
      ...
      if (mounted) message(context, 'แก้ไขข้อมูลไม่สำเร็จ: $e');
    }
  }
...
class _AmuletGroupPageState extends State<AmuletGroupPage> {
  ...
  Future<void> refresh() async {
    ...
    setState(() {
      group = all.where((e) => groupKey(e) == key).toList();
    });
  }
  ...
  Future<void> addReference() async {
    ...
    try {
      ...
      if (mounted) message(context, 'เพิ่มรายการอ้างอิงไม่สำเร็จ: $e');
    }
  }
  ...
  Future<void> deleteItem(AmuletData item) async {
    ...
    message(
      context,
      'ลบลำดับอ้างอิงที่ ${item.realItemNumber} แล้ว',
    );
  ...
  Future<void> backup() async {
    ...
    try {
      ...
      final data = {
        ...
        'nextRealItemNumber': prefs.getInt(nextNumberKey) ?? 1,
      ...
      if (mounted) message(context, 'สำรองข้อมูลไม่สำเร็จ: $e');
    } finally {
  ...
  Future<void> importData() async {
    ...
    try {
      ...
      for (final e in decoded['data']) {
        if (e is Map) {
          items.add(
            AmuletData.fromMap(Map<String, dynamic>.from(e)),
          );
        }
      }
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt(nextNumberKey, next);

      if (mounted) {
        message(context, 'นำเข้าข้อมูล ${items.length} รายการเรียบร้อยแล้ว');
      }
    } catch (e) {
      if (mounted) message(context, 'นำเข้าข้อมูลไม่สำเร็จ: $e');
    } finally {
      if (mounted) setState(() => working = false);
    }
  }
        ...
        body: ListView(
          ...
          children: [
            ...
            SwitchListTile(
              ...
              subtitle: const Text('หลังบันทึกการสแกนให้เลือกพื้นที่ถัดไป'),
              value: autoNext,
              onChanged: (v) async {
                setState(() => autoNext = v);
...
class _ScanPageState extends State<ScanPage> {
                      ...
                      Row(
                        children: [
                          IconButton(
                            ...
                            onPressed:
                                scanning ? null : () => Navigator.pop(sheet),
                          ),
                          Expanded(
                            child: Text(
                              'สแกน $area',
  ...
  Future<void> pickImage() async {
    try {
      ...
      if (picked != null) await saveScan('นำเข้าจากโทรศัพท์');
    } catch (e) {
  ...
  Future<void> saveScan(String method) async {
    ...
    show(
      'บันทึกอัตโนมัติแล้ว • $oldArea • สแกนครั้งที่ $n',
    );
  }
