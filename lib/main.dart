          ...
          _button(
            ...
            () async {
              await Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) =>
                      CreateGroupPage(widget.cameras),
                ),
              );
              load();
            },
  ...
  Future<void> create() async {
    ...
    if (name.isEmpty || temple.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'กรุณากรอกชื่อกลุ่มและวัด / สำนัก',
          ),
        ),
      );
      return;
...
class GroupListPage extends StatefulWidget {
  ...
  @override
  State<GroupListPage> createState() =>
      _GroupListPageState();
}
...
class ModelListPage extends StatefulWidget {
  ...
  @override
  State<ModelListPage> createState() =>
      _ModelListPageState();
}
...
class _CreateDataPageState extends State<CreateDataPage> {
  ...
  Future<void> save() async {
    ...
    if (typeName.isEmpty || typeName == 'อื่น ๆ') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('กรุณาระบุชนิด'),
        ),
      );
      return;
    }
    ...
    final index =
        groups.indexWhere((e) => e.id == widget.groupId);
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'รุ่น *',
              hintText:
                  'เช่น 1, 2500, รุ่นแรก, กรุวัดใหญ่',
              border: OutlineInputBorder(),
            ),
          ),
...
class TypeListPage extends StatefulWidget {
  ...
  @override
  State<TypeListPage> createState() =>
      _TypeListPageState();
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: type!.references.isEmpty
          ? const Center(
              child: Text('ยังไม่มีข้อมูลอ้างอิง'),
            )
          : ListView.builder(
              padding: const EdgeInsets.all(12),
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  child: ListTile(
                    title: Text(
                      'อ้างอิงที่ ${ref.referenceNumber}',
                    ),
                    subtitle: Text(
...
class _ScanPageState extends State<ScanPage> {
  ...
  String statusText(String area) {
    return results[area]?.status ?? 'ยังไม่ได้ระบุ';
  }
