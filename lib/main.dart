...
class ScanResult {
  final String area,status,quality,details;
  final double brightness,sharpness;
  final bool fromGallery;
  ScanResult({
    required this.area,this.status='มีข้อมูล',this.quality='',
    this.details='',this.brightness=0,this.sharpness=0,this.fromGallery=false,
  });
  Map<String,dynamic> toMap()=>{
    'area':area,'status':status,'quality':quality,'details':details,
    'brightness':brightness,'sharpness':sharpness,'fromGallery':fromGallery,
  };
  factory ScanResult.fromMap(Map<String,dynamic> m)=>ScanResult(
    area:m['area']??'',status:m['status']??'มีข้อมูล',
    quality:m['quality']??'',details:m['details']??'',
...
class _GroupListPageState extends State<GroupListPage> {
  ...
  Future<void> deleteGroup(GroupData g) async {
    if(!await confirmDelete(context,'ลบรายการ','ต้องการลบรายการนี้ใช่หรือไม่?'))return;
      ...
      itemBuilder:(_,i){
        ...
        return Card(child:ListTile(
          ...
          onTap:()=>Navigator.push(c,MaterialPageRoute(
            builder:(_)=>ModelListPage(cameras:widget.cameras,groupId:g.id),
          )).then((_)=>load()),
...
class ModelListPage extends StatefulWidget {
  ...
  const ModelListPage({required this.cameras,required this.groupId,super.key});
...
class _ModelListPageState extends State<ModelListPage> {
  ...
  Future<void> load() async {
    ...
    for(final g in gs){
      if(g.id==widget.groupId&&mounted){setState(()=>group=g);return;}
    }
  }
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(title:Text(g?.name??'รายการ'),actions:[
        IconButton(onPressed:addModel,icon:const Icon(Icons.add),tooltip:'เพิ่มรุ่น'),
      ]),
    ...
    for(final g in groups){
      ...
      for(final m in g.models){
        ...
        for(final savedType in m.types){
          ...
          savedType.references.add(ref);
          Storage.renumber(savedType);
          await Storage.save(groups);
          if(!mounted)return;
          await Navigator.push(context,MaterialPageRoute(
            builder:(_)=>ScanPage(
              cameras:widget.cameras,groupId:widget.groupId,
              modelId:widget.modelId,typeId:widget.typeId,referenceId:ref.id)));
          load();return;
        }
      }
    }
            ...
            itemBuilder:(_,i){
              ...
              return Card(
                child:ListTile(
                  ...
                  subtitle:Text(
                    'มีข้อมูล ${ref.scans.length}/${areas.length} ด้าน'),
                  trailing:PopupMenuButton<String>(
                    onSelected:(v){
...
class _ScanPageState extends State<ScanPage> {
  ...
  Future<void> scanCamera() async {
    ...
    try{
      ...
      await saveScan(file,fromGallery:false);
  ...
  Future<void> scanGallery() async {
    ...
    try{
      ...
      if(file==null)return;
      await saveScan(file,fromGallery:true);
    }catch(_){
  ...
  Future<void> saveScan(XFile file,{required bool fromGallery}) async {
    final groups=await Storage.load();
    for(final g in groups){
      if(g.id!=widget.groupId)continue;
      for(final m in g.models){
        ...
        for(final t in m.types){
          ...
          for(var i=0;i<t.references.length;i++){
            ...
            await Storage.save(groups);
            if(mounted)setState(()=>scans[selectedArea]=result);
