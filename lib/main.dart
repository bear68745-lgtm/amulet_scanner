...
class AiTestResult {
  ...
  factory AiTestResult.fromMap(Map<String,dynamic> m)=>AiTestResult(
    ...
    reason:m['reason']??'',checkedAt:m['checkedAt']??''
  );
}
...
class Storage {
  static const keys=[
    'reference_groups','ai_knowledge','ai_learning_history',
    'ai_memory_tests','ai_test_results'
  ];
  ...
  static Future<void> _save<T>(
    String key,List<T> list,Map<String,dynamic> Function(T) f,
  ) async {
    final p=await SharedPreferences.getInstance();
  ...
  static Future<void> deleteAiByReference(String id) async {
    final k=await knowledge(),h=await history(),
        t=await tests(),r=await results();
    final ids=k.where((e)=>e.referenceId==id).map((e)=>e.id).toSet();

    await saveKnowledge(k.where((e)=>e.referenceId!=id).toList());
    await saveHistory(h.where((e)=>e.referenceId!=id).toList());
    await saveTests(t.where((e)=>
      e.referenceId!=id&&!ids.contains(e.knowledgeId)).toList());
    await saveResults(r.where((e)=>
      e.referenceId!=id&&!ids.contains(e.knowledgeId)).toList());
  }
      ...
      for(final m in g.models){
        ...
        for(final t in m.types)
          for(final p in t.prints)
            for(final r in p.references) ids.add(r.id);
      }
    }
    return ids;
  }
      ...
      for(final m in g.models)
        for(final t in m.types)
          if(t.id==id)
            for(final p in t.prints)
              for(final r in p.references) ids.add(r.id);
    for(final r in ids) await deleteAiByReference(r);
  }
  ...
  static Future<void> deleteAiByGroup(String id) async {
    final k=await knowledge(),h=await history(),
        t=await tests(),r=await results();
    final gids=k.where((e)=>e.groupId==id).map((e)=>e.id).toSet();
    final refs=<String>{};
    ...
    for(final g in await groups()){
      ...
      for(final m in g.models)
        for(final t in m.types)
          for(final p in t.prints)
            for(final r in p.references) refs.add(r.id);
    }

    await saveKnowledge(k.where((e)=>e.groupId!=id).toList());
    await saveHistory(h.where((e)=>e.groupId!=id).toList());
    await saveTests(t.where((e)=>
      !refs.contains(e.referenceId)&&!gids.contains(e.knowledgeId)).toList());
    await saveResults(r.where((e)=>
      !refs.contains(e.referenceId)&&!gids.contains(e.knowledgeId)).toList());
  }
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> addGroup() async {
    final n=TextEditingController(),t=TextEditingController();
        ...
        content:Column(
          ...
          children:[
            ...
            TextField(
              controller:t,
              decoration:const InputDecoration(
                labelText:'วัด / เกจิ',border:OutlineInputBorder()),
            ),
          ],
        ),
        actions:[
    ...
    if(ok==true){
      ...
      groups.add(GroupData(
        id:newId(),name:n.text.trim(),temple:t.text.trim(),
        createdAt:d,updatedAt:d,models:[],
      ));
  ...
  Future<void> editGroup(GroupData g) async {
    final n=TextEditingController(text:g.name),
        t=TextEditingController(text:g.temple);
    final ok=await showDialog<bool>(
      context:context,
      builder:(_)=>AlertDialog(
    ...
    if(ok==true){
      g.name=n.text.trim();g.temple=t.text.trim();
      await Storage.saveGroup(g);await load();
    }
    n.dispose();t.dispose();
  }
            ...
            itemBuilder:(_,i){
              ...
              return Card(
                child:ListTile(
                  ...
                  subtitle:Text(g.temple.isEmpty?'ยังไม่ได้ระบุวัด':g.temple),
                  ...
                  onTap:() async {
                    await Navigator.push(c,MaterialPageRoute(
                      builder:(_)=>ModelPage(
                        group:g,cameras:widget.cameras,autoCreate:true),
                    ));
                    await load();
                  },
                ),
...
class ModelPage extends StatefulWidget {
  ...
  const ModelPage({
    required this.group,required this.cameras,
    this.autoCreate=false,super.key});
...
class _ModelPageState extends State<ModelPage> {
  ...
  void initState(){
    ...
    if(widget.autoCreate)
      WidgetsBinding.instance.addPostFrameCallback((_){
        if(mounted&&!opened){opened=true;add();}
      });
  }

  Future<void> add()=>textDialog(context,'สร้างรุ่น','',(v) async {
    final d=now();
    widget.group.models.add(ModelData(
      id:newId(),name:v,createdAt:d,updatedAt:d,types:[]));
    await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  });

  Future<void> edit(ModelData m)=>textDialog(context,'แก้ไขรุ่น',m.name,(v) async {
    m.name=v;await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  });
  ...
  Widget build(BuildContext c)=>Scaffold(
              ...
              itemBuilder:(_,i){
                ...
                return Card(child:ListTile(
                  ...
                  onTap:() async {
                    await Navigator.push(c,MaterialPageRoute(
                      builder:(_)=>TypePage(
                        group:widget.group,model:m,
                        cameras:widget.cameras,autoCreate:true),
                    ));
                    if(mounted)setState((){});
                  },
                ));
    ...
    floatingActionButton:FloatingActionButton.extended(
      onPressed:add,icon:const Icon(Icons.add),label:const Text('สร้างรุ่น')),
  );
}
...
class TypePage extends StatefulWidget {
  ...
  final bool autoCreate;
  const TypePage({
    required this.group,required this.model,required this.cameras,
    this.autoCreate=false,super.key});
...
class _TypePageState extends State<TypePage> {
  ...
  Future<void> add() async {
    ...
    selected=await showDialog<String>(
      ...
      builder:(_)=>AlertDialog(
        ...
        content:DropdownButtonFormField<String>(
          ...
          decoration:const InputDecoration(
            border:OutlineInputBorder(),labelText:'ชนิดพระ'),
        ),
        actions:[
          TextButton(
          ...
          ElevatedButton(
            onPressed:()=>selected==null
                ? null:Navigator.pop(context,selected),
            child:const Text('สร้าง')),
        ],
      ),
    );
    ...
    widget.model.types.add(TypeData(
      id:newId(),name:selected!,createdAt:d,updatedAt:d,prints:[]));
    await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  }

  Future<void> edit(TypeData t)=>textDialog(context,'แก้ไขชนิด',t.name,(v) async {
    t.name=v;await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  });
  ...
  Widget build(BuildContext c)=>Scaffold(
              ...
              itemBuilder:(_,i){
                ...
                return Card(child:ListTile(
                  ...
                  onTap:() async {
                    await Navigator.push(c,MaterialPageRoute(
                      builder:(_)=>PrintPage(
                        group:widget.group,model:widget.model,type:t,
                        cameras:widget.cameras,autoCreate:true),
                    ));
                    if(mounted)setState((){});
                  },
    ...
    floatingActionButton:FloatingActionButton.extended(
      onPressed:add,icon:const Icon(Icons.add),label:const Text('สร้างชนิด')),
  );
}
...
class PrintPage extends StatefulWidget {
  ...
  final bool autoCreate;
  const PrintPage({
    required this.group,required this.model,required this.type,
    required this.cameras,this.autoCreate=false,super.key});
...
class _PrintPageState extends State<PrintPage> {
  ...
  Future<void> add()=>textDialog(context,'สร้างพิมพ์','',(v) async {
    final d=now();
    widget.type.prints.add(PrintData(
      id:newId(),name:v,createdAt:d,updatedAt:d,references:[]));
    await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  });

  Future<void> edit(PrintData p)=>textDialog(context,'แก้ไขพิมพ์',p.name,(v) async {
    p.name=v;await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  });

  Future<void> remove(PrintData p) async {
    if(!await confirmDelete(
      context,'ลบพิมพ์?','องค์อ้างอิงและความจำ AI ของพิมพ์นี้จะถูกลบ',
    ))return;
  ...
  Widget build(BuildContext c)=>Scaffold(
    ...
    body:Column(children:[
      PathBar([
        widget.group.name,widget.model.name,widget.type.name,'พิมพ์']),
      Expanded(
          ...
          : ListView.builder(
              ...
              itemBuilder:(_,i){
                ...
                return Card(child:ListTile(
                  title:Text(p.name,
                    style:const TextStyle(fontWeight:FontWeight.bold)),
                  subtitle:Text('องค์อ้างอิง ${p.references.length} องค์'),
                  trailing:Row(
                    mainAxisSize:MainAxisSize.min,
                    children:[
                      IconButton(
                  ...
                  onTap:() async {
                    await Navigator.push(c,MaterialPageRoute(
                      builder:(_)=>ReferencePage(
                        ...
                        print:p,cameras:widget.cameras,autoCreate:true),
                    ));
                    if(mounted)setState((){});
                  },
                ));
    ...
    floatingActionButton:FloatingActionButton.extended(
      onPressed:add,icon:const Icon(Icons.add),label:const Text('สร้างพิมพ์')),
  );
}
...
class ReferencePage extends StatefulWidget {
  ...
  const ReferencePage({
    required this.group,required this.model,required this.type,
    required this.print,required this.cameras,
    this.autoCreate=false,super.key});
...
class _ReferencePageState extends State<ReferencePage> {
  ...
  void initState(){
    ...
    if(widget.autoCreate)
      WidgetsBinding.instance.addPostFrameCallback((_){
        if(mounted&&!opened){opened=true;addReference();}
      });
  }
  ...
  Future<void> addReference() async {
    ...
    for(final r in widget.print.references)
      if(r.referenceNumber>max)max=r.referenceNumber;

    final d=now();
    widget.print.references.add(ReferenceData(
      id:newId(),referenceNumber:max+1,
      createdAt:d,updatedAt:d,scans:[]));
    await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  }
  ...
  Future<void> editReference(ReferenceData r) async {
    await Navigator.push(context,MaterialPageRoute(
      builder:(_)=>ReferenceEditPage(
        group:widget.group,model:widget.model,type:widget.type,
        print:widget.print,reference:r,cameras:widget.cameras),
    ));
    await Storage.saveGroup(widget.group);
    if(mounted)setState((){});
  }
  ...
  Widget build(BuildContext c){
    ...
    return Scaffold(
      ...
      body:Column(children:[
        PathBar([
          widget.group.name,widget.model.name,widget.type.name,
          widget.print.name,'องค์อ้างอิง']),
        Expanded(
                ...
                itemBuilder:(_,i){
                  ...
                  return Card(
                    child:ExpansionTile(
                      ...
                      children:[
                        ...areas.map((a){
                          final s=r.scans.cast<ScanResult?>().firstWhere(
                            (e)=>e?.area==a,orElse:()=>null);
                          return ListTile(
                            title:Text(a),
                            subtitle:Text(
                        ...
                        Padding(
                          ...
                          child:SizedBox(
                            ...
                            child:ElevatedButton.icon(
                              onPressed:()=>editReference(r),
                              icon:const Icon(Icons.edit),
                              label:const Text('แก้ไขข้อมูลทุกด้าน'),
                            ),
                          ),
                        ),
                      ],
...
class ReferenceEditPage extends StatefulWidget {
  ...
  const ReferenceEditPage({
    required this.group,required this.model,required this.type,
    required this.print,required this.reference,required this.cameras,
    super.key});
...
class _ReferenceEditPageState extends State<ReferenceEditPage> {
  ...
  Future<void> scanArea(String area) async {
    final r=await Navigator.push<ScanResult>(
      ...
      MaterialPageRoute(
        builder:(_)=>ScanPage(
          cameras:widget.cameras,area:area,
          group:widget.group,model:widget.model,type:widget.type,
          print:widget.print,reference:widget.reference),
      ),
    );

    if(r==null)return;
    final old=getScan(area);
    if(old==null)widget.reference.scans.add(r);
    else old.details=r.details;
  ...
  Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(
        title:Text('แก้ไของค์อ้างอิง #${widget.reference.referenceNumber}')),
      body:Column(children:[
        PathBar([
          widget.group.name,widget.model.name,widget.type.name,
          widget.print.name,
          'องค์อ้างอิง #${widget.reference.referenceNumber}','แก้ไข']),
        Expanded(
          child:ListView(
            padding:const EdgeInsets.all(10),
            children:[
                ...
                child:Padding(
                  ...
                  child:Column(
                    ...
                    children:[
                      ...
                      const Text('แก้ไขและสแกนข้อมูลขององค์เดิมได้ทุกด้าน'),
                    ],
                  ),
                ),
              ),
...
class ScanPage extends StatelessWidget {
  ...
  const ScanPage({
    required this.cameras,required this.area,
    required this.group,required this.model,required this.type,
    required this.print,required this.reference,super.key});
      ...
      final r=await Navigator.push<ScanResult>(
        c,MaterialPageRoute(
          builder:(_)=>AiVisionPage(
            imageFile:f,area:area,group:group,model:model,
            type:type,print:print,reference:reference)),
      );
  ...
  Future<void> camera(BuildContext c) async {
    ...
    final f=await Navigator.push<XFile>(
      c,MaterialPageRoute(
        builder:(_)=>CameraScanPage(cameras:cameras,area:area)),
    );
    if(f==null)return;
  ...
  Widget build(BuildContext c)=>Scaffold(
    ...
    body:Padding(
      ...
      child:Column(children:[
        PathBar([
          group.name,model.name,type.name,print.name,
          'องค์อ้างอิง #${reference.referenceNumber}',area]),
...
class CameraScanPage extends StatefulWidget {
  ...
  const CameraScanPage({
    required this.cameras,required this.area,super.key});
...
class _CameraScanPageState extends State<CameraScanPage> {
  ...
  Future<void> capture() async {
    ...
    try{
      ...
      if(mounted)ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content:Text('ไม่สามารถถ่ายภาพได้')));
    }
  ...
  Widget build(BuildContext c){
    final x=controller;
    return Scaffold(
      appBar:AppBar(title:Text('ถ่ายภาพ${widget.area}')),
      body:x==null||!x.value.isInitialized
        ? const Center(child:CircularProgressIndicator())
        : Column(children:[
            Expanded(child:CameraPreview(x)),
            Padding(
...
class AiVisionPage extends StatefulWidget {
  ...
  const AiVisionPage({
    required this.imageFile,required this.area,
    required this.group,required this.model,required this.type,
    required this.print,required this.reference,super.key});
...
class _AiVisionPageState extends State<AiVisionPage> {
  ...
  void initState(){
    ...
    String old='';
    for(final s in widget.reference.scans)
      if(s.area==widget.area)old=s.details;

    for(final h in aiHeads)c[h]=TextEditingController();
  ...
  Widget build(BuildContext cxt)=>Scaffold(
    appBar:AppBar(title:Text('รายละเอียด${widget.area}')),
    body:Column(children:[
      Expanded(
        child:ListView(
          padding:const EdgeInsets.all(10),
