...
const amuletObservationPoints = {
  ...
  'พระสมเด็จ': [
    ...
    'คราบและร่องรอยตามธรรมชาติ',
    'ร่องรอยการสร้าง',
    'ลักษณะที่ใช้เปรียบเทียบกับสาย/แหล่งสร้างอื่น',
    'ความผิดปกติที่ควรนำไปเปรียบเทียบกับองค์อ้างอิง'
  ],
...
Future<List<AmuletData>> loadAllItems() async {
  ...
  try {
    ...
    if (z != null && z.isNotEmpty) {
      final list =
          jsonDecode(utf8.decode(gzip.decode(base64Decode(z)))) as List;
      return list
...
Future<void> saveAllItems(List<AmuletData> items) async {
  final p = await prefs();
  final z = base64Encode(
      gzip.encode(utf8.encode(jsonEncode(items.map((e) => e.toMap()).toList()))));
        ...
        body: ListView(
          ...
          children: [
            ...
            tile(
              c,
              'สำรอง / นำเข้าข้อมูล',
              'สำรองหรือนำข้อมูลกลับเข้าเครื่อง',
              Icons.import_export,
              const BackupPage(),
            ),
            tile(
              c,
              'ตั้งค่า',
              'ตั้งค่าการสแกนและการแสดงข้อมูล',
              Icons.settings,
              const SettingsPage(),
            ),
          ],
...
class _SavedListPageState extends State<SavedListPage> {
  ...
  Map<String, List<AmuletData>> groups() {
    ...
    for (final e in items) {
      final q =
          '${e.name} ${e.model} ${e.type} ${e.temple}'.toLowerCase();
...
class _EditGroupPageState extends State<EditGroupPage> {
  ...
  void initState() {
    ...
    type = amuletTypes.contains(a.type)
        ? a.type
        : amuletTypes.first;
  }
  ...
  Widget build(BuildContext c) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              value: amuletTypes.contains(type)
                  ? type
                  : amuletTypes.first,
              decoration: const InputDecoration(
                labelText: 'ชนิดพระ',
...
class _ScanPageState extends State<ScanPage> {
  ...
  Future<void> loadSettings() async {
    ...
    setState(() {
      realObject =
          p.getBool('setting_real_object_default') ?? true;
      history =
          p.getBool('setting_show_scan_history') ?? true;
      autoNext =
          p.getBool('setting_auto_next_area') ?? true;
    });
  }
  ...
  Future<void> start(String a) async {
    ...
    if (realObject && widget.cameras.isNotEmpty) {
      ...
      if (f != null) {
        await saveScan('นำเข้าจากโทรศัพท์');
      }
    }
  }
  ...
  Future<void> openCamera() async {
    ...
    try {
      ...
      if (mounted) {
        msg(context, 'เปิดกล้องไม่สำเร็จ: $e');
      }
    } finally {
      try {
        await camera?.stopImageStream();
    ...
    final s = ScanData(
      ...
      details: {
        ...
        'authenticityDecision': {
          ...
          'result': null,
          'note':
              'ขนาดและสัดส่วนใช้เป็นหลักฐานประกอบ '
              'ไม่ตัดสินแท้/เก๊เพียงอย่างเดียว'
        },
      },
      imageFeatures: {
        'modelName': '',
  ...
  Widget build(BuildContext c) {
    final list = areas();
    final completed =
        list.where((e) => count(e) > 0).length;
    final observations =
        observationPointsFor(widget.item.type);
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'ชนิด: ${widget.item.type.isEmpty ? 'อื่น ๆ' : widget.item.type}',
                  ),
                  const SizedBox(height: 6),
                  Text(
                    'ความคืบหน้า $completed/${list.length} จุด',
                  ),
                ],
              ),
            ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  const Text(
                    'ระบบเตรียมข้อมูลสำหรับ AI '
                    'โดยยังไม่เก็บรูปภาพต้นฉบับ',
                  ),
                  const SizedBox(height: 10),
                  OutlinedButton.icon(
                    onPressed: () => go(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  const Text(
                    'AI สามารถสร้าง Reference Model '
                    'จากข้อมูลอ้างอิงหลายแหล่งได้ โดยไม่ใช้ขนาดพิกเซลของรูปแทนขนาดจริง',
                  ),
                  const SizedBox(height: 6),
                  const Text(
...
class _BackupPageState extends State<BackupPage> {
  Future<void> backup() async {
    ...
    if (mounted) {
      msg(context, 'สำรองข้อมูลเรียบร้อย');
    }
  }
  ...
  Future<void> restore() async {
    ...
    try {
      ...
      if (mounted) {
        msg(context, 'นำเข้าข้อมูลเรียบร้อย');
      }
    } catch (e) {
      if (mounted) {
        msg(context, 'ไฟล์ไม่ถูกต้อง');
      }
    }
  }
  ...
  Future<void> deleteAll() async {
    ...
    if (mounted) {
      msg(context, 'ลบข้อมูลทั้งหมดแล้ว');
    }
  }
  ...
  Widget build(BuildContext c) => Scaffold(
        appBar: AppBar(
          title: const Text('สำรอง / นำเข้าข้อมูล'),
        ),
        body: ListView(
          padding: const EdgeInsets.all(12),
          children: [
            ...
            Card(
              child: ListTile(
                ...
                title:
                    const Text('ลบข้อมูลทั้งหมด'),
                subtitle: const Text(
                  'ลบรายการที่บันทึก และประวัติการสแกนทั้งหมด',
                ),
                onTap: deleteAll,
              ),
            ),
...
class AiReconstructionPage
  ...
  Widget build(BuildContext c) {
    final embedding =
        scan.imageFeatures['embedding'];

    final active =
        embedding is List && embedding.isNotEmpty;

    return Scaffold(
      appBar: AppBar(
        title: const Text('AI Reconstruction'),
      ),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          ...
          const Card(
            child: Padding(
              padding: EdgeInsets.all(14),
              child: Text(
                'หน้านี้เป็นโหมดเตรียมระบบ AI สำหรับ '
                'MediaPipe/LiteRT ยังไม่ใช่ผลวิเคราะห์จริง '
                'และภาพต้นฉบับไม่ได้ถูกบันทึก',
              ),
            ),
          ),
        ],
...
class AiReconstructionPainter
  ...
  void paint(Canvas c, Size s) {
    ...
    if (active) {
      c.drawCircle(
        Offset(s.width * .42, s.height * .45),
        5,
        p,
      );

      c.drawCircle(
        Offset(s.width * .58, s.height * .45),
        5,
        p,
      );
    }
