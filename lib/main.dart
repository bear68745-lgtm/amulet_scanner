...
String groupKey(AmuletData e) {
  return '${e.name.trim().toLowerCase()}|||${e.model.trim().toLowerCase()}';
}
  ...
  Widget build(BuildContext c) {
    final menu = [
      ...
      [
        ...
        Colors.green,
        () => go(c, SavedListPage(cameras: cameras)),
      ],
      [
        Icons.import_export,
        'สำรอง / นำเข้าข้อมูล',
...
class _CreateDataPageState extends State<CreateDataPage> {
  ...
  Future<void> save() async {
    try {
      ...
      if (mounted) {
        msg(context, 'บันทึกข้อมูลไม่สำเร็จ: $e');
      }
    }
  }
...
class _SavedListPageState extends State<SavedListPage> {
  ...
  Future<void> deleteGroup(List<AmuletData> g) async {
    ...
    if (mounted) {
      setState(() => items = all);
      msg(context, 'ลบข้อมูลกลุ่มทั้งหมดแล้ว');
    }
  }
  ...
  Widget build(BuildContext c) {
    ...
    final filtered = groups.values.where((g) {
      ...
      ..sort((a, b) => a.first.id.compareTo(b.first.id));
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  final changed =
                                    ...
                                    c,
                                    MaterialPageRoute(
                                      builder: (_) => EditGroupPage(group: g),
                                    ),
                                  );
...
class _EditGroupPageState extends State<EditGroupPage> {
  ...
  void initState() {
    ...
    type = amuletTypes.contains(e.type)
        ? e.type
        : amuletTypes.first;
  }
  ...
  Future<void> save() async {
    try {
      ...
      msg(
        context,
        'แก้ไขข้อมูลหลักและรายการอ้างอิง '
        '${widget.group.length} รายการแล้ว',
      );

      Navigator.pop(context, true);
    } catch (e) {
      if (mounted) {
        msg(context, 'แก้ไขไม่สำเร็จ: $e');
      }
    }
  }
        ...
        children: [
          Card(
            child: Padding(
              ...
              child: Text(
                'แก้ไขข้อมูลหลัก\n'
                'ข้อมูลนี้จะใช้กับรายการอ้างอิงทั้งหมด '
                '${widget.group.length} รายการ\n\n'
...
class _AmuletGroupPageState extends State<AmuletGroupPage> {
  ...
  Future<void> refresh() async {
    ...
    final u = x.where((e) => ids.contains(e.id)).toList()
      ..sort((a, b) => a.id.compareTo(b.id));
  ...
  Future<void> deleteGroup() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'ลบข้อมูลหลักและรายการอ้างอิงทั้งหมด '
          '${group.length} รายการหรือไม่?\n\n'
  ...
  Future<void> deleteReference(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'ต้องการลบรายการอ้างอิง $n หรือไม่?\n\n'
    ...
    try {
      ...
      msg(context, 'ลบรายการอ้างอิง $n แล้ว');
    } catch (e) {
      if (mounted) {
        msg(context, 'ลบรายการอ้างอิงไม่สำเร็จ: $e');
      }
    }
  }
  ...
  Future<void> addReference() async {
    ...
    try {
      ...
      if (mounted) {
        msg(context, 'เพิ่มรายการอ้างอิงไม่สำเร็จ: $e');
      }
    }
  }
  ...
  Future<void> backup() async {
    ...
    try {
      ...
      if (mounted && path != null) {
        msg(context, 'สำรองข้อมูลเรียบร้อยแล้ว');
      }
    } catch (e) {
      if (mounted) {
        msg(context, 'สำรองข้อมูลไม่สำเร็จ: $e');
      }
    } finally {
      if (mounted) {
  ...
  Future<void> importData() async {
    ...
    try {
      ...
      if (mounted) {
        msg(
          context,
          'นำเข้าข้อมูล ${x.length} รายการเรียบร้อยแล้ว',
        );
      }
    } catch (e) {
      if (mounted) {
        msg(context, 'นำเข้าข้อมูลไม่สำเร็จ: $e');
      }
    } finally {
      if (mounted) {
        setState(() => working = false);
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  const Text(
                    'ภาพจริงไม่ได้ถูกเก็บไว้ในเครื่อง',
                  ),
                ],
              ),
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  row(
                    'พื้นผิว',
                    countInfo(
                      scan.details['surfaceDetails'],
                    ),
                  ),
                  row(
                    'ตำหนิ',
                    countInfo(
                      scan.details['defects'],
                    ),
                  ),
                  row(
                    'เส้นหล่อ',
                    countInfo(
                      scan.details['castingLines'],
                    ),
                  ),
                  row(
                    'รายละเอียดพิมพ์',
                    countInfo(
                      scan.details['patternDetails'],
                    ),
                  ),
                  row(
                    'ข้อสังเกต',
                    countInfo(
                      scan.details['observations'],
                    ),
                  ),
                  row(
                    'ความมั่นใจ',
                    scan.details['aiConfidence']
                            ?.toString() ??
                        'ยังไม่มี',
                  ),
                ],
            ...
            child: Padding(
              ...
              child: Text(
                hasEmbedding
                    ...
                    : 'ตอนนี้ยังเป็นโหมดเตรียมระบบ '
                      'ภาพจำลองนี้ยังไม่ใช่ผลจาก AI วิเคราะห์ภาพจริง',
              ),
            ),
          ),
          const SizedBox(height: 8),
