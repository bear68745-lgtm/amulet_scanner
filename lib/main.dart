...
class HomePage extends StatelessWidget{
  ...
  Widget tile(
    BuildContext c,String t,String s,IconData i,Widget p,
  )=>Card(
    child:ListTile(
                ...
                children:gs.entries.map((g){
                  ...
                  return Card(
                    child:ListTile(
                      ...
                      title:Text(
                        a.name.isEmpty?'ไม่ระบุชื่อ':a.name,
                      ),
                      subtitle:Text(
  ...
  Future<void> deleteItem(AmuletData item)async{
    final ok=await showDialog<bool>(
      ...
      builder:(c)=>AlertDialog(
        ...
        content:const Text(
          'ต้องการลบรายการองค์จริงนี้หรือไม่?',
        ),
        actions:[
          TextButton(
            onPressed:()=>Navigator.pop(c,false),
          ...
          child:Padding(
            ...
            child:Column(
              ...
              children:[
                ...
                Text(
                  'รุ่น: ${widget.group.first.model.isEmpty?'ไม่ระบุ':
                    widget.group.first.model}',
                ),
                Text(
                  'ชนิด: ${widget.group.first.type.isEmpty?'ไม่ระบุ':
...
class _ScanPageState extends State<ScanPage>{
  ...
  int count(String a)=>
      widget.item.scans.where((e)=>e.area==a).length;

  List<String> areas(){
    final x=<String>[...scanAreas];

    for(final s in widget.item.scans){
      if(s.area.isNotEmpty&&!x.contains(s.area)){
        x.add(s.area);
      }
    }
            ...
            child:Padding(
              ...
              child:Column(
                ...
                children:[
                  ...
                  Text(
                    'รุ่น: ${widget.item.model.isEmpty?'ไม่ระบุ':
                      widget.item.model}',
                  ),
                  Text(
                    'ชนิด: ${widget.item.type.isEmpty?'อื่น ๆ':
                      widget.item.type}',
                  ),
                  const SizedBox(height:6),
                  ...
                  Wrap(
                    ...
                    children:[
                      FilledButton.icon(
                        ...
                        label:Text(
                          realObject?'สแกนองค์จริง':'เริ่มสแกน',
                        ),
                      ),
                      OutlinedButton.icon(
                        onPressed:()async{
            ...
            child:Padding(
              ...
              child:Column(
                ...
                children:[
                  ...
                  const SizedBox(height:8),
                  const Text(
                    'สิ่งที่ระบบ AI ควรตรวจสอบจากภาพสแกน',
                  ),
                  const SizedBox(height:10),
                  Wrap(
                    spacing:6,
      ...
      children:[
        ...
        Card(
          child:ListTile(
            ...
            title:const Text('นำเข้าข้อมูล'),
            subtitle:const Text(
              'นำข้อมูลจากไฟล์ JSON กลับเข้าเครื่อง',
            ),
            onTap:restore,
          ),
        ),
