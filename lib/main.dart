...
const _aiFront='front_scan',
    _aiBack='back_scan',
    _aiSideBottom='side_bottom_scan',
    _aiDetails='all_visible_details',
...
Map<String,dynamic> toAiData(AmuletData e)=> {
  ...
  'aiInstruction':
    ...
    'ไม่จำกัดเฉพาะจุด หากไม่ชัดให้ระบุว่าตรวจสอบไม่ได้ '
      ...
      children:[
          ...
          child:Padding(
            ...
            child:Column(
              ...
              children:[
                ...
                const Text(
                  'พื้นฐาน 5+ องค์เป็นเพียงตัวเตือน '
                  'เพิ่มรายการอ้างอิงได้ไม่จำกัด'
                )
              ]
            )
          )
        ...
        ...group.map((e)=>Card(
          child:ListTile(
            ...
            subtitle:Text(
              'สแกนแล้ว ${scannedAreas(e)}/${scanAreas.length} พื้นที่'
            ),
            trailing:PopupMenuButton<String>(
              onSelected:(v)async{
...
class _ScanPageState extends State<ScanPage>{
            ...
            content:SizedBox(
              ...
              child:Column(
                children:[
                  ...
                  const Text(
                    'ภาพใช้ชั่วคราวใน RAM และไม่บันทึกเป็นรูปภาพ'
                  )
                ]
              )
            ),
  ...
  Future<void> saveScan(String method)async{
    ...
    if(autoNext)setState(()=>area=nextAutoArea());
    else setState((){});
  }
              ...
              children:[
                ...
                SizedBox(
                  ...
                  child:FilledButton.icon(
                    ...
                    label:Text(
                      realObject?'สแกน $area':'นำข้อมูลจากโทรศัพท์'
                    )
                  )
                ),
            ...
            child:Column(
              ...
              children:[
                ...
                Text(
                  ...
                  'ที่มองเห็นได้จากภาพสแกน โดยไม่จำกัดเฉพาะจุด'
                ),
                SizedBox(height:6),
                Text(
                  'ถ้ารายละเอียดไม่ชัด จะระบุว่า '
                  '“ตรวจสอบไม่ได้ / ภาพไม่ละเอียดพอ” '
                  'และจะไม่เดาว่าไม่มีรายละเอียด'
                ),
                SizedBox(height:6),
                Text(
                  'ข้อมูลจากทุกพื้นที่และทุกอ้างอิงสามารถนำมาเปรียบเทียบร่วมกันได้'
                )
              ]
            )
          )
...
class _BackupPageState extends State<BackupPage>{
  ...
  Future<void> restore()async{
    try{
      ...
      if(file.bytes!=null)data=utf8.decode(file.bytes!);
      else if(file.path!=null)data=await File(file.path!).readAsString();
    ...
    body:ListView(
      children:[
        ...
        SwitchListTile(
          ...
          subtitle:const Text(
            'ลำดับหลัก: ด้านหน้า → ด้านหลัง → ด้านข้าง/ก้นพระ'
          ),
          value:autoNext,
          onChanged:(v)async{
            setState(()=>autoNext=v);
