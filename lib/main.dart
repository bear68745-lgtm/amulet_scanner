...
class Storage {
  ...
  // เลขอ้างอิงนับใหม่ภายในชนิดนั้น
  static void renumber(TypeData type) {
    for (var i = 0; i < type.references.length; i++) {
      final old = type.references[i];

      type.references[i] = old.copyWith(
        referenceNumber: i + 1,
      );
    }
  }
  ...
  Future<void> create() async {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('กรุณาใส่ชื่อกลุ่ม'),
        ),
      );
      return;
    }
...
class _TypeListPageState
    extends State<TypeListPage> {
  ModelData? model;
...
class _ReferenceListPageState
  ...
  Future<void> addReference() async {
    ...
    final number = type!.references.length + 1;
