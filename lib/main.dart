...
List<String> observationPointsFor(String type) {
  return amuletObservationPoints[type] ??
...
class ScanData {
  ...
  factory ScanData.fromMap(Map<String, dynamic> m) {
    return ScanData(
      area: '${m['area'] ?? ''}',
      method: '${m['method'] ?? ''}',
...
class _CreateDataPageState
    extends State<CreateDataPage> {
...
class _EditGroupPageState
  ...
  late TextEditingController pim;
  late TextEditingController temple;
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'รุ่น: '
                    '${widget.group.first.model.isEmpty ? 'ไม่ระบุ' : widget.group.first.model}',
                  ),

                  Text(
                    'ชนิด: '
                    '${widget.group.first.type.isEmpty ? 'ไม่ระบุ' : widget.group.first.type}',
                  ),
          ...
          ...group.map(
            (e) => Card(
              child: ListTile(
                ...
                subtitle: Text(
                  'สแกนแล้ว '
                  '${e.scans.length}/${scanAreas.length} จุด',
                ),
                trailing:
                    PopupMenuButton<String>(
                  onSelected: (v) async {
    ...
    final s = ScanData(
      ...
      details: {
        ...
        'referenceModel': {
          ...
          'sourceType':
              'reference_data',
          'isActualObjectMeasurement':
              false,
          'imagePixelSizeIsNotRealSize':
              true,
          'size': {
            'height': null,
            'width': null,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'รุ่น: '
                    '${widget.item.model.isEmpty ? 'ไม่ระบุ' : widget.item.model}',
                  ),

                  Text(
                    'ชนิด: '
                    '${widget.item.type.isEmpty ? 'อื่น ๆ' : widget.item.type}',
                  ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'ยังไม่ใช่การวัดองค์จริง '
                    'และจะไม่ใช้ขนาดเพียงอย่างเดียว '
                    'ในการตัดสินความแท้',
                  ),
                ],
              ),
              ...
              child: Row(
                children: [
                  ...
                  const Expanded(
                    child: Text(
                      'ความเป็นส่วนตัว: '
                      'ภาพจากกล้องหรือโทรศัพท์ใช้ชั่วคราว '
                      'ไม่บันทึกรูปภาพ '
                      'และไม่เก็บไฟล์ภาพลงฐานข้อมูล',
                    ),
                  ),
                ],
              ),
  ...
  Future<void> deleteAll() async {
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: const Text(
          'ข้อมูลพระ รายการอ้างอิง '
          'และประวัติการสแกนทั้งหมดจะถูกลบ',
        ),
        actions: [
          TextButton(
            onPressed: () =>
...
class AiReconstructionPage
  ...
  Widget build(BuildContext c) {
    ...
    final active = embedding is List &&
        ...
        children: [
          ...
          const Card(
            child: Padding(
              padding: EdgeInsets.all(14),
              child: Text(
                'หน้านี้เป็นโหมดเตรียมระบบ AI '
                'สำหรับ MediaPipe/LiteRT '
                'ยังไม่ใช่ผลวิเคราะห์จริง '
