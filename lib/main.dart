...
class AiKnowledge {
  ...
  factory AiKnowledge.fromMap(Map<String,dynamic> m)=>AiKnowledge(
    ...
    typeId:m['typeId']??'',printId:m['printId']??'',
...
class Storage {
  ...
  static Future<void> addAi({
    ...
    final i=list.indexWhere((e)=>e.referenceId==reference.id&&e.area==scan.area);
    final now=DateTime.now();

    final item=AiKnowledge(
      id:i>=0?list[i].id:newId(),
      groupId:group.id,modelId:model.id,typeId:type.id,printId:print.id,
      referenceId:reference.id,area:scan.area,content:content,
      createdAt:i>=0?list[i].createdAt:now,updatedAt:now,
    );
  ...
  static Future<void> deleteAi({
    ...
    final list=await loadAi();
    list.removeWhere((e)=>e.referenceId==referenceId&&e.area==area);
    await saveAi(list);
  }
  ...
  static Future<List<AiLearningHistory>> loadHistory() async {
    ...
    try {
      return (jsonDecode(r) as List)
          .map((e)=>AiLearningHistory.fromMap(Map<String,dynamic>.from(e)))
          .toList();
    } catch(_){return[];}
  }
  ...
  static Future<void> saveHistory(List<AiLearningHistory> data) async {
    final p=await SharedPreferences.getInstance();
    await p.setString(historyKey,jsonEncode(data.map((e)=>e.toMap()).toList()));
  }
  ...
  static Future<void> saveTests(List<AiMemoryTest> data) async {
    final p=await SharedPreferences.getInstance();
    await p.setString(testKey,jsonEncode(data.map((e)=>e.toMap()).toList()));
  }
  ...
  static Future<void> saveResults(List<AiTestResult> data) async {
    final p=await SharedPreferences.getInstance();
    await p.setString(resultKey,jsonEncode(data.map((e)=>e.toMap()).toList()));
  }
...
Set<String> refsOfPrint(PrintData p)=>p.references.map((e)=>e.id).toSet();
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> createGroup() async {
    final name=TextEditingController(),temple=TextEditingController();
    ...
    final n=name.text.trim(),t=temple.text.trim();
    name.dispose();temple.dispose();
    if(ok!=true||n.isEmpty)return;

    final group=GroupData(
      id:newId(),name:n,temple:t,createdAt:DateTime.now(),
    );
  ...
  Future<void> editGroup(GroupData g) async {
    final name=await textDialog(
      context,title:'แก้ไขชื่อพระ',label:'ชื่อพระ',initial:g.name,
    );
    if(name==null)return;

    final temple=await textDialog(
      context,title:'แก้ไขวัด/พระเกจิ',
      label:'วัด/พระเกจิ',initial:g.temple,
    );
    if(temple==null)return;

    g.name=name;g.temple=temple;g.updatedAt=DateTime.now();
    await Storage.save(groups);
    if(mounted)setState((){});
  }
...
class _ModelPageState extends State<ModelPage> {
  Future<void> addModel() async {
    final name=await textDialog(
      context,title:'เพิ่มรุ่น',label:'ชื่อรุ่น',
    );
    if(name==null)return;

    final model=ModelData(
      id:newId(),name:name,createdAt:DateTime.now(),
    );
  ...
  Future<void> editModel(ModelData m) async {
    final name=await textDialog(
      context,title:'แก้ไขรุ่น',label:'ชื่อรุ่น',initial:m.name,
    );
    if(name==null)return;

    m.name=name;m.updatedAt=DateTime.now();
    await Storage.save(widget.groups);
    if(mounted)setState((){});
  }
...
class _TypePageState extends State<TypePage> {
  Future<void> addType() async {
    ...
    final type=TypeData(
      id:newId(),name:selected,createdAt:DateTime.now(),
    );
  ...
  Future<void> editType(TypeData t) async {
    final name=await textDialog(
      context,title:'แก้ไขชนิด',label:'ชื่อชนิด',initial:t.name,
    );
    if(name==null)return;

    t.name=name;t.updatedAt=DateTime.now();
    await Storage.save(widget.groups);
    if(mounted)setState((){});
  }
...
class _PrintPageState extends State<PrintPage> {
  ...
  Future<void> editPrint(PrintData p) async {
    final name=await textDialog(
      context,title:'แก้ไขพิมพ์',
      label:'ชื่อพิมพ์',initial:p.name,
    );
    if(name==null)return;

    p.name=name;p.updatedAt=DateTime.now();
    await Storage.save(widget.groups);
    if(mounted)setState((){});
  }
...
class _ReferencePageState extends State<ReferencePage> {
  ...
  Future<void> editArea(ReferenceData ref,String area) async {
    ...
    final result=ScanResult(area:area,details:value);
            ...
            itemBuilder:(_,i){
              ...
              return Card(
                child:ExpansionTile(
                  ...
                  subtitle:Text('ชนิด: ${widget.type.name}'),
                  trailing:PopupMenuButton<String>(
                    onSelected:(v){
                  ...
                  children:scanAreas.map((area){
                    /* แก้ Error สำคัญตรงนี้:
                       scan -> foundScan */
                    final matches=ref.scans.where((e)=>e.area==area);
                    final foundScan=
                        matches.isEmpty?null:matches.first;

                    final hasData=foundScan!=null&&
                        foundScan.details.trim().isNotEmpty;
                    ...
                    return ListTile(
                      ...
                      trailing:Icon(
                        hasData?Icons.edit:Icons.camera_alt,
                      ),
                      onTap:()=>hasData
                          ? editArea(ref,area)
                          : scan(ref,area),
...
class _AiVisionPageState extends State<AiVisionPage> {
  ...
  void selectAll(bool value){
    setState((){
      for(final h in aiHeads)selected[h]=value;
    });
  }
  ...
  @override
  Widget build(BuildContext context)=>Scaffold(
    appBar:AppBar(title:Text('AI วิเคราะห์ ${widget.area}')),
    body:ListView(
      padding:const EdgeInsets.all(12),
      children:[
        ClipRRect(
