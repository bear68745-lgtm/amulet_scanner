...
class ScanData {
  ...
  // เตรียมไว้สำหรับ MediaPipe + LiteRT
  // เก็บเฉพาะคุณลักษณะของภาพ
  // ไม่เก็บไฟล์รูปภาพ
...
class _SavedListPageState extends State<SavedListPage> {
  ...
  Widget build(BuildContext c) {
    ...
    final filtered = groups.values.where((g) {
      ...
      ..sort(
        (a, b) => a.first.id.compareTo(b.first.id),
      );
            ...
            Card(
              child: Padding(
                ...
                child: Text(
                  ...
                  'ข้อมูลสแกนและรายละเอียด AI '
                  'ของแต่ละรายการจะยังคงอยู่',
                  style:
...
class _AmuletGroupPageState
  ...
  Future<void> deleteReference(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: Text(
          'ลบรายการอ้างอิง $refNo',
        ),
        content: Text(
          'ต้องการลบรายการอ้างอิง $refNo หรือไม่?\n\n'
          'ข้อมูลสแกนและข้อมูล AI ของรายการอ้างอิงนี้ '
          'จะถูกลบด้วย\n\n'
    ...
    try {
      ...
      msg(
        context,
        'ลบรายการอ้างอิง $refNo แล้ว',
      );
    } catch (e) {
      if (mounted) {
        msg(
          context,
          'ลบรายการอ้างอิงไม่สำเร็จ: $e',
        );
      }
    }
  }
  ...
  Future<void> saveScan(
    ...
    widget.item.scans.add(
      ScanData(
        ...
        // เตรียมข้อมูลสำหรับ MediaPipe + LiteRT
        // ตอนนี้ยังไม่มี embedding จริง
        // และไม่มีการเก็บไฟล์รูปภาพ
        imageFeatures: {
          'modelName': '',
          'modelVersion': '',
          'featureVersion': 1,
