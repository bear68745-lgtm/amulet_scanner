...
class ReferenceStorage {
  ...
  static List<ReferenceData> _toList(List data) => data
  ...
  static Future<void> deleteGroup(String name, String model) async {
    final list = await load();
    list.removeWhere((x) => x.name == name && x.model == model);
    await _write(list);
  }

  static Future<int> nextReferenceNumber() async {
    final p = await SharedPreferences.getInstance();
    final n = (p.getInt('last_reference_number') ?? 0) + 1;
    await p.setInt('last_reference_number', n);
    return n;
  }

  static Future<void> _write(List<ReferenceData> list) async {
    final p = await SharedPreferences.getInstance();
    final text = jsonEncode(list.map((x) => x.toMap()).toList());
    final encoded = base64Encode(gzip.encode(utf8.encode(text)));
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> loadData() async {
    ...
    if (mounted) setState(() => allData = data);
  }

  List<List<ReferenceData>> getGroups() {
    final map = <String, List<ReferenceData>>{};
    for (final x in allData) {
      map.putIfAbsent('${x.name}|||${x.model}', () => []).add(x);
    }
  ...
  Future<void> deleteGroup(List<ReferenceData> group) async {
    ...
    await ReferenceStorage.deleteGroup(x.name, x.model);
    await loadData();
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () => open(
                  CreateReferencePage(cameras: widget.cameras),
                ),
              ),
            ),
            const SizedBox(height: 20),
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          x.name.isEmpty ? 'ไม่ระบุชื่อพระ' : x.name,
...
class _ReferenceListPageState extends State<ReferenceListPage> {
  ...
  Future<void> loadReferences() async {
    ...
    final list = all
        .where(
          (x) => x.name == widget.name && x.model == widget.model,
        )
        .toList()
      ..sort(
    ...
    if (mounted) setState(() => references = list);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('รายการองค์อ้างอิง')),
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text('มีองค์อ้างอิง ${references.length} องค์'),
                  ],
                ),
              ),
            ),
            ...
            for (final x in references) buildReferenceCard(x),
          ],
        ),
      ),
    );
      ...
      child: InkWell(
        onTap: () async {
          await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => ReferenceDetailPage(reference: x),
            ),
          );
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('ข้อมูลสแกน ${x.scans.length} รายการ'),
                  ],
                ),
              ),
              const Icon(Icons.arrow_forward_ios, size: 18),
...
class ReferenceDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('องค์อ้างอิงที่ ${reference.referenceNumber}'),
      ),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          ...
          if (reference.scans.isEmpty)
            const Card(
              child: Padding(
                ...
                child: Text('ยังไม่มีข้อมูลจากการสแกน'),
              ),
            ),
        ],
      ),
  ...
  Widget infoRow(String title, String value) => Padding(
...
class _EditReferencePageState extends State<EditReferencePage> {
  ...
  void initState() {
    super.initState();
    selectedType =
        types.contains(widget.type) ? widget.type : null;
  }
...
class _ScanPageState extends State<ScanPage> {
  ...
  Future<void> startCamera() async {
    if (widget.cameras.isEmpty) {
      if (mounted) setState(() => loadingCamera = false);
      return;
    }
  ...
  void selectArea(int i) {
    if (!saving) setState(() => currentArea = i);
  }
  ...
  Future<void> takePhoto() async {
    ...
    try {
      await c.takePicture();
      await saveScanResult(fromGallery: false);
    } catch (_) {
      if (mounted) msg('ถ่ายภาพไม่สำเร็จ');
    }
  }
  ...
  Future<void> chooseGallery() async {
    ...
    try {
      ...
      await saveScanResult(fromGallery: true);
    } catch (_) {
      if (mounted) msg('เลือกรูปไม่สำเร็จ');
    }
  }
  ...
  Future<void> saveReference() async {
    if (saving || results.isEmpty) return;
  ...
  Widget areaButton(int index) {
    ...
    final selected = currentArea == index;
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('สแกนองค์อ้างอิง'),
          actions: [
            TextButton.icon(
            ...
            Expanded(
              child: initialized
                  ...
                  : loadingCamera
                      ...
                      : const Center(
                          child: Text('ไม่สามารถเปิดกล้องได้'),
                        ),
            ),
            SafeArea(
              top: false,
