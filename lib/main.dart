      ...
      ListView.builder(
        ...
        itemBuilder:(_,i){
          ...
          final refs=m.types.fold<int>(
            0,(s,t)=>s+t.prints.fold<int>(
              0,(a,p)=>a+p.references.length));
          return Card(child:ListTile(
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(title:Text('รุ่น ${m?.name??''}'),actions:[
        FilledButton.icon(
          onPressed:addType,icon:const Icon(Icons.add,size:22),
      ...
      ListView.builder(
        ...
        itemBuilder:(_,i){
          ...
          return Card(child:ListTile(
            ...
            subtitle:Text(
              'พิมพ์ ${type.prints.length} รายการ\nอ้างอิง $refs องค์'),
            trailing:Row(mainAxisSize:MainAxisSize.min,children:[
              IconButton(
                icon:const Icon(Icons.remove_circle_outline),
...
class _PrintListPageState extends State<PrintListPage>{
    ...
    for(final g in gs){
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          ...
          final now=Storage.now();
          t.prints.add(PrintData(
            id:Storage.id(),name:name,createdAt:now,updatedAt:now));
          await Storage.save(gs);load();return;
        }
      }
    }
  ...
  Future<void>deletePrint(PrintData p)async{
    ...
    final scans=p.references.fold<int>(
      0,(s,r)=>s+r.scans.length);
    final msg=refs>0
      ...
      ListView.builder(
        ...
        itemBuilder:(_,i){
          ...
          return Card(child:ListTile(
            title:Text(p.name),
            subtitle:Text(
              'อ้างอิง ${p.references.length} องค์\nข้อมูลสแกน $scans ด้าน'),
            trailing:Row(mainAxisSize:MainAxisSize.min,children:[
              IconButton(
                icon:const Icon(Icons.remove_circle_outline),
...
class _ReferenceListPageState extends State<ReferenceListPage>{
  ...
  Future<void>addReference()async{
    ...
    final ref=ReferenceData(
      id:Storage.id(),referenceNumber:p.references.length+1,
    ...
    return Scaffold(
      appBar:AppBar(title:Text(p?.name??'อ้างอิง'),actions:[
        Padding(
          padding:const EdgeInsets.only(right:8),
          child:FilledButton.icon(
            onPressed:addReference,icon:const Icon(Icons.add,size:24),
...
class ScanPage extends StatefulWidget{
  ...
  const ScanPage({
    required this.cameras,required this.groupId,
    required this.modelId,required this.typeId,
    required this.printId,required this.referenceId,super.key});
    ...
    body:ListView(padding:const EdgeInsets.all(16),children:[
      ...
      Card(child:Padding(
        ...
        child:Column(children:[
          ...
          Text(
            'เลือก $selectedArea แล้วใช้กล้อง หรือเลือกรูปจากแกลเลอรี',
            textAlign:TextAlign.center),
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      ...
      appBar:AppBar(
        ...
        title:const Text('ถ่ายภาพอ้างอิง')),
      body:error!=null
