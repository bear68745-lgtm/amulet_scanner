...
// เก็บ "เลขถัดไป" ที่เคยถูกใช้จริง
// เพื่อป้องกันเลขที่ลบไปแล้วถูกนำกลับมาใช้ซ้ำ
...
///
/// หลักการ:
/// 1. ตรวจเลขสูงสุดจากข้อมูลจริง
/// 2. ตรวจเลขถัดไปที่เคยถูกบันทึกไว้
/// 3. ใช้ค่าที่สูงกว่า
///
/// ทำแบบนี้เพื่อให้
/// - ข้อมูลจริงไม่ขัดกับตัวนับ
/// - เลขที่ลบไปแล้วไม่ถูกนำกลับมาใช้
Future<int> nextReferenceNumber(
  ...
  for (final item in items) {
    if (item.realItemNumber > maxExisting) {
      maxExisting = item.realItemNumber;
    }
  }
  ...
  final calculatedNext =
      maxExisting + 1;

  final next = storedNext > calculatedNext
      ? storedNext
      : calculatedNext;

  // ถ้าตัวนับเดิมต่ำกว่าข้อมูลจริง ให้ซ่อมตัวนับ
  if (storedNext != next) {
    await prefs.setInt(
      nextNumberKey,
      next,
...
/// ยืนยันว่าเลขถัดไปต้องไม่ย้อนกลับ
Future<void> commitReferenceNumber(
  int number,
) async {
  final prefs =
      await SharedPreferences.getInstance();
  ...
  final requiredNext = number + 1;

  if (requiredNext > current) {
    await prefs.setInt(
      nextNumberKey,
      requiredNext,
    );
...
/// ทำให้ตัวนับสัมพันธ์กับข้อมูลจริง
///
/// สำคัญ:
/// ไม่ลดเลขลงเมื่อมีการลบข้อมูล
/// เพื่อป้องกันการนำเลขเดิมกลับมาใช้ซ้ำ
...
///
/// ถ้าพบ:
/// - เลขอ้างอิงเป็น 0 หรือติดลบ
/// - เลขอ้างอิงซ้ำ
///
/// จะกำหนดเลขใหม่เฉพาะรายการที่ผิด
Future<bool> repairReferenceNumbers(
  ...
  for (final item in items) {
    final number = item.realItemNumber;
  ...
  if (finalNext < maxExisting + 1) {
    finalNext = maxExisting + 1;
  }

  if (finalNext < storedNext) {
    finalNext = storedNext;
  }
...
Future<List<AmuletData>> loadAllItems() async {
  ...
  if (compressed != null &&
    ...
    try {
      ...
      if (data is List) {
        ...
        // ตรวจเลขอ้างอิงของข้อมูลเดิม
        final repaired =
            await repairReferenceNumbers(
          items,
        );
      ...
      debugPrint(
        'โหลดข้อมูลบีบอัดไม่สำเร็จ: $e',
      );
    }
  }

  final old =
      prefs.getStringList(oldDataKey);
  ...
  try {
    ...
    if (repaired) {
      debugPrint(
        'ซ่อมลำดับอ้างอิงข้อมูลเก่าแล้ว',
      );
    }
...
class _CreateDataPageState
  ...
  Future<void> save() async {
    try {
      ...
      // เลขอ้างอิงใหม่จากข้อมูลจริง
      final number =
          await nextReferenceNumber(
        items,
      );
      ...
      // ยืนยันเลขที่เพิ่งใช้
      await commitReferenceNumber(
        number,
      );
      ...
      message(
        ...
        widget.isAddingReference
            ? 'เพิ่มรายการอ้างอิงเรียบร้อย '
                '• ลำดับอ้างอิงที่ $number'
...
class _SavedListPageState
  ...
  Widget build(
    ...
    // เรียงเลขอ้างอิงภายในแต่ละกลุ่ม
    for (final group
        in groups.values) {
      group.sort(
        (a, b) =>
            a.realItemNumber.compareTo(
          b.realItemNumber,
        ),
      );
    }
...
class EditDataPage
  ...
  @override
  State<EditDataPage> createState() =>
      _EditDataPageState();
}

class _EditDataPageState
    extends State<EditDataPage> {
  ...
  Future<void> save() async {
    try {
      ...
      items[i] = AmuletData(
        ...
        // สำคัญมาก:
        // แก้ข้อมูลแล้วเลขอ้างอิงเดิมต้องอยู่
        realItemNumber:
            old.realItemNumber,
      ...
      message(
        context,
        'แก้ไขลำดับอ้างอิงที่ '
        '${old.realItemNumber} แล้ว',
      );
...
class _AmuletGroupPageState
  ...
  Future<void> addReference() async {
    ...
    try {
      ...
      // ใช้เลขอ้างอิงถัดไปจากข้อมูลจริง
      final number =
          await nextReferenceNumber(
        all,
      );
  ...
  Future<void> deleteItem(
    ...
    try {
      ...
      // saveAllItems จะไม่ลด nextNumberKey
      // ดังนั้นเลขที่ลบแล้วจะไม่กลับมาใช้ซ้ำ
...
class _BackupPageState
  ...
  Future<void> backup() async {
    ...
    try {
      ...
      // ให้เลขถัดไปสัมพันธ์กับข้อมูลก่อนสำรอง
      await syncReferenceNumber(
        items,
      );
  ...
  Future<void> importData() async {
    ...
    try {
      ...
      // ซ่อมเลขอ้างอิงในไฟล์สำรองก่อนนำเข้า
      await repairReferenceNumbers(
        items,
      );
      ...
      // หาเลขสูงสุดจากข้อมูลที่นำเข้า
      int maxImported = 0;

      for (final item
          in items) {
        if (item.realItemNumber >
            maxImported) {
          maxImported =
              item.realItemNumber;
        }
      ...
      // ใช้ค่าที่สูงที่สุด
      //
      // เพื่อไม่ให้เลขที่เคยใช้บนเครื่อง
      // ถูกนำกลับมาใช้ซ้ำหลัง Import
      int finalNext =
          currentNext;

      if (backupNext >
          finalNext) {
        finalNext = backupNext;
      }
      ...
      // หลัง save ต้องตั้งตัวนับอีกครั้ง
      // เพราะการ Import ต้องรักษาเลขที่เคยใช้บนเครื่อง
      await prefs.setInt(
        nextNumberKey,
        finalNext,
      );
      ...
      if (mounted) {
        message(
          context,
          'นำเข้าข้อมูล '
          '${items.length} รายการเรียบร้อยแล้ว',
        );
      }
    } catch (e) {
      if (mounted) {
