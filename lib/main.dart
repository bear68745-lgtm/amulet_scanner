...
class ScanData {
  ...
  // เตรียมพื้นที่สำหรับ AI
  // เพื่อไม่ต้องรื้อโครงสร้างภายหลัง
...
Map<String, dynamic> toAiData(
  ...
  return {
    ...
    // ข้อมูลสแกนทั้งหมดขององค์นี้
    'scans': item.scans.map(
      (scan) {
        return {
          'area': scan.area,
...
class _AmuletGroupPageState
  ...
  Future<void> _addReference() async {
    ...
    try {
      ...
      // ใช้ข้อมูลหลักเดิมของกลุ่ม
      // แล้วสร้าง "องค์อ้างอิง" ใหม่
      final newReference =
          AmuletData(
        id: DateTime.now()
            .millisecondsSinceEpoch,
      ...
      // สำคัญ:
      // + อ้างอิง ไม่เปิด CreateDataPage
      // แต่ไปหน้า ScanPage โดยตรง
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => ScanPage(
      ...
      // กลับมารายการหลักเดิม
      // แล้วโหลดข้อมูลล่าสุดทั้งหมด
      final updatedItems =
          await loadAllItems();

      final groupKey =
          '${base.name.trim().toLowerCase()}|||'
          '${base.model.trim().toLowerCase()}';
...
class _ScanPageState
  ...
  Future<void> _pickFromPhone() async {
    try {
      ...
      // ใช้ไฟล์ชั่วคราวเท่านั้น
      // ไม่เก็บ path หรือไฟล์รูปในฐานข้อมูล
  ...
  Future<void> _saveScan(
    ...
    // ==============================================
    // เตรียมข้อมูลสำหรับ AI ตั้งแต่ตอนนี้
    // ==============================================
    ...
    final scan = ScanData(
      ...
      details: {
        ...
        // ให้ AI รู้ว่าเป็นข้อมูลจากพื้นที่ใด
        ...
        // เปิดทางให้ AI เชื่อมโยงกับข้อมูลสแกนก่อนหน้า
        'canCompareWithPreviousScans':
            true,

        // ช่องสำหรับผลวิเคราะห์ในอนาคต
        'surfaceDetails': [],
        'defects': [],
        'moldDetails': [],
        'castingLines': [],
