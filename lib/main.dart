...
class PrintData{
  ...
  PrintData({
    ...
    required this.updatedAt,List<ReferenceData>? references,
...
class TypeData{
  ...
  TypeData({
    required this.id,required this.name,required this.createdAt,
    required this.updatedAt,List<PrintData>? prints,
  }):prints=prints??[];
...
class ModelData{
  ...
  ModelData({
    required this.id,required this.name,required this.createdAt,
    required this.updatedAt,List<TypeData>? types,
  }):types=types??[];
...
class GroupData{
  ...
  GroupData({
    ...
    required this.createdAt,required this.updatedAt,List<ModelData>? models,
  }):models=models??[];
...
class AiKnowledge{
  ...
  factory AiKnowledge.fromMap(Map<String,dynamic> m)=>AiKnowledge(
    id:m['id']??newId(),groupId:m['groupId']??'',modelId:m['modelId']??'',
    typeId:m['typeId']??'',printId:m['printId']??'',referenceId:m['referenceId']??'',
    area:m['area']??'',content:m['content']??'',createdAt:m['createdAt']??now(),
    updatedAt:m['updatedAt']??now(),
  );
}
...
class AiLearningHistory{
  ...
  factory AiLearningHistory.fromMap(Map<String,dynamic> m)=>AiLearningHistory(
    id:m['id']??newId(),groupId:m['groupId']??'',modelId:m['modelId']??'',
    typeId:m['typeId']??'',printId:m['printId']??'',referenceId:m['referenceId']??'',
    area:m['area']??'',content:m['content']??'',learnedAt:m['learnedAt']??now(),
  );
}
...
class AiMemoryTest{
  ...
  factory AiMemoryTest.fromMap(Map<String,dynamic> m)=>AiMemoryTest(
    ...
    question:m['question']??'',answer:m['answer']??'',testedAt:m['testedAt']??now(),
  );
}
...
class AiTestResult{
  ...
  factory AiTestResult.fromMap(Map<String,dynamic> m)=>AiTestResult(
    id:m['id']??newId(),testId:m['testId']??'',knowledgeId:m['knowledgeId']??'',
    referenceId:m['referenceId']??'',area:m['area']??'',mode:m['mode']??'',
    result:m['result']??'',answer:m['answer']??'',referenceAnswer:m['referenceAnswer']??'',
    reason:m['reason']??'',checkedAt:m['checkedAt']??now(),
  );
}
...
class Storage{
  ...
  static Future<void> saveGroup(GroupData g)async{
    final a=await groups(),i=a.indexWhere((e)=>e.id==g.id);
  ...
  static Future<void> clearAiByScope({
    String? groupId,String? modelId,String? typeId,String? printId,
    Set<String>? referenceIds,
  })async{
    final refs=referenceIds??{};
    final k=await knowledge(),h=await history();
    final tests=await Storage.tests(),results=await Storage.results();
    ...
    final matchedT=tests.where((e)=>ref.contains(e.referenceId)||kid.contains(e.knowledgeId)).toList();
    ...
    await Future.wait([
      saveKnowledge(k),saveHistory(h),saveTests(tests),saveResults(results),
    ]);
  }
  ...
  static Future<void> rebuildAi(
    ...
    await clearAiByScope(
      groupId:groupId,modelId:modelId,typeId:typeId,
      printId:printId,referenceIds:refIds(list),
    );
    ...
    for(final x in list)
      for(final s in x.reference.scans){
        ...
        k.add(AiKnowledge(
          id:kid,groupId:x.group.id,modelId:x.model.id,
          typeId:x.type.id,printId:x.print.id,
          referenceId:x.reference.id,area:s.area,content:content,
          createdAt:d,updatedAt:d,
        ));

        h.add(AiLearningHistory(
          id:newId(),groupId:x.group.id,modelId:x.model.id,
          typeId:x.type.id,printId:x.print.id,
          referenceId:x.reference.id,area:s.area,
          content:content,learnedAt:d,
        ));
      }
  ...
  static Future<void> syncModel(GroupData g,ModelData m)=>
      rebuildAi(links(_copyGroup(g,[m])),modelId:m.id);

  static Future<void> syncType(
    GroupData g,ModelData m,TypeData t,
  )=>rebuildAi(
    links(_copyGroup(g,[
      ModelData(
  ...
  static Future<void> syncPrint(
    GroupData g,ModelData m,TypeData t,PrintData p,
  )=>rebuildAi(
    links(_copyGroup(g,[
      ModelData(
        id:m.id,name:m.name,createdAt:m.createdAt,updatedAt:m.updatedAt,
  ...
  static Future<void> syncScan({
    ...
    k.add(AiKnowledge(
      id:kid,groupId:group.id,modelId:model.id,typeId:type.id,
      printId:print.id,referenceId:reference.id,area:scan.area,
      content:content,createdAt:d,updatedAt:d,
    ));
  ...
  static Future<void> deletePrint(PrintData p)=>
      clearAiByScope(printId:p.id,referenceIds:_refsFromPrint(p));

  static Future<void> deleteType(TypeData t)=>
      clearAiByScope(typeId:t.id,referenceIds:_refsFromType(t));

  static Future<void> deleteModel(ModelData m)=>
      clearAiByScope(modelId:m.id,referenceIds:_refsFromModel(m));

  static Future<void> deleteGroup(GroupData g)=>
      clearAiByScope(groupId:g.id,referenceIds:refIds(links(g)));
  ...
  static Set<String> _refsFromModel(ModelData m)=>
      m.types.expand((t)=>t.prints).expand((p)=>p.references).map((r)=>r.id).toSet();
}

GroupData _copyGroup(GroupData g,List<ModelData> models)=>GroupData(
  id:g.id,name:g.name,temple:g.temple,
  createdAt:g.createdAt,updatedAt:g.updatedAt,models:models,
);
...
class _CreateReferencePageState extends State<CreateReferencePage>{
  ...
  Future<int> nextNumber(List<GroupData> groups)async{
    ...
    for(final g in groups)
      for(final x in Storage.links(g))
        if(x.reference.referenceNumber>max)max=x.reference.referenceNumber;
    return max+1;
  }
  ...
  Future<void> create()async{
    ...
    try{
      ...
      if(group==null){
        group=GroupData(
          id:newId(),name:groupName,temple:temple,
          createdAt:d,updatedAt:d,
        );
        groups.add(group);
      }
      ...
      ModelData? model=firstWhereOrNull(
        group.models,(m)=>m.name.trim()==modelName,
      );

      if(model==null){
        model=ModelData(
          id:newId(),name:modelName,createdAt:d,updatedAt:d,
        );
        group.models.add(model);
      }

      TypeData? type=firstWhereOrNull(
        model.types,(t)=>t.name.trim()==selectedType,
      );

      if(type==null){
        type=TypeData(
          id:newId(),name:selectedType,createdAt:d,updatedAt:d,
        );
        model.types.add(type);
      }

      PrintData? print=firstWhereOrNull(
        type.prints,(p)=>p.name.trim()==printName,
      );

      if(print==null){
        print=PrintData(
          id:newId(),name:printName,createdAt:d,updatedAt:d,
        );
      ...
      if(mounted){
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content:Text('ไม่สามารถสร้างข้อมูลได้: $e')),
        );
      }
    }finally{
      ...
      children:[
        ...
        TextField(
          ...
          decoration:const InputDecoration(
            labelText:'ชื่อกลุ่ม',border:OutlineInputBorder(),
          ),
        ),
        const SizedBox(height:12),
        TextField(
        ...
        SizedBox(
          ...
          child:ElevatedButton.icon(
            ...
            label:Text(saving?'กำลังสร้าง...':'สร้างองค์อ้างอิง'),
          ),
        ),
      ],
    ),
...
class _HomePageState extends State<HomePage>{
  ...
  Future<void> createGroup()async{
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder:(_)=>CreateReferencePage(cameras:widget.cameras),
      ),
    );
    if(mounted)load();
  }
  ...
  Future<void> deleteGroup(GroupData g)async{
    ...
    await Storage.clearAiByScope(
      groupId:g.id,referenceIds:ids,
    );
    await load();
  }

  int countReferences(GroupData g)=>g.models.fold(
    0,(a,m)=>a+m.types.fold(
      0,(b,t)=>b+t.prints.fold(
        0,(c,p)=>c+p.references.length,
      ),
    ),
  );
  ...
  int countScans(GroupData g)=>g.models.fold(
    0,(a,m)=>a+m.types.fold(
      0,(b,t)=>b+t.prints.fold(
        0,(c,p)=>c+p.references.fold(
          0,(d,r)=>d+r.scans.length,
        ),
      ),
    ),
  );
                    ...
                    return Card(
                      ...
                      child:ListTile(
                        ...
                        onTap:()=>Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:(_)=>ModelPage(
                              cameras:widget.cameras,group:g,
                            ),
                          ),
                        ).then((_)=>load()),
                        trailing:Row(
...
class ModelPage extends StatefulWidget{
  ...
  final GroupData group;
  const ModelPage({
    super.key,required this.cameras,required this.group,
  });
...
class _ModelPageState extends State<ModelPage>{
  Future<void> add()async{
    await textDialog(context,'เพิ่มรุ่น','',(v)async{
      final d=now();
      widget.group.models.add(
        ModelData(
          id:newId(),name:v,createdAt:d,updatedAt:d,
        ),
      );
  ...
  Future<void> edit(ModelData m)async{
    await textDialog(context,'แก้ไขรุ่น',m.name,(v)async{
      m.name=v;
      m.updatedAt=now();
...
class TypePage extends StatefulWidget{
  ...
  const TypePage({
    super.key,required this.cameras,
    required this.group,required this.model,
  });
...
class _TypePageState extends State<TypePage>{
  ...
  Widget build(BuildContext context)=>Scaffold(
    ...
    body:Column(
      children:[
        PathBar(
          '${widget.group.name} > ${widget.model.name} > ชนิด',
        ),
        Expanded(
...
class PrintPage extends StatefulWidget{
  ...
  const PrintPage({
    super.key,required this.cameras,required this.group,
    required this.model,required this.type,
  });
...
class _PrintPageState extends State<PrintPage>{
  Future<void> add()async{
    await textDialog(context,'เพิ่มพิมพ์','',(v)async{
      final d=now();
  ...
  Future<void> edit(PrintData p)async{
    await textDialog(context,'แก้ไขพิมพ์',p.name,(v)async{
      p.name=v;
      p.updatedAt=now();
  ...
  Future<void> delete(PrintData p)async{
    ...
    final ids=p.references.map((r)=>r.id).toSet();
                  ...
                  itemBuilder:(_,i){
                    ...
                    return Card(
                      child:ListTile(
                        title:Text(p.name),
                        subtitle:Text(
                          'องค์อ้างอิง ${p.references.length}',
                        ),
                        onTap:()=>Navigator.push(
                          context,
                          MaterialPageRoute(
...
class ReferencePage extends StatefulWidget{
  ...
  const ReferencePage({
    super.key,required this.cameras,required this.group,
    required this.model,required this.type,required this.print,
  });
...
class _ReferencePageState extends State<ReferencePage>{
  ...
  Future<void> deleteReference(ReferenceData r)async{
    if(!await confirmDelete(
      context,'องค์อ้างอิง #${r.referenceNumber}',
    ))return;
    ...
    await Storage.saveGroup(widget.group);
    await Storage.clearAiByScope(referenceIds:{r.id});
...
class _ReferenceEditPageState extends State<ReferenceEditPage>{
  ScanResult? getScan(String area)=>
      firstWhereOrNull(
        widget.reference.scans,(s)=>s.area==area,
      );
...
class ScanPage extends StatelessWidget{
  ...
  const ScanPage({
    super.key,required this.cameras,required this.area,
    required this.group,required this.model,required this.type,
    required this.print,required this.reference,
  });
    ...
    body:Center(
      child:Column(
        ...
        children:[
          ...
          const SizedBox(height:20),
          ElevatedButton.icon(
            onPressed:cameras.isEmpty?null:()async{
              final r=await Navigator.push<ScanResult>(
                context,
                MaterialPageRoute(
...
class _CameraScanPageState extends State<CameraScanPage>{
  ...
  Future<void> take()async{
    final c=controller;
    if(busy||c==null||!c.value.isInitialized)return;
...
class AiVisionPage extends StatefulWidget{
  ...
  const AiVisionPage({
    super.key,required this.imageFile,required this.area,
    required this.group,required this.model,required this.type,
    required this.print,required this.reference,
  });
    ...
    if(old!=null){
      ...
      for(final h in aiHeads){
        final line=firstWhereOrNull(
          lines,(e)=>e.startsWith('$h:'),
        );
        if(line!=null){
          controllers[h]!.text=
              line.substring(h.length+1).trim();
        }
      }
