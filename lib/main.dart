...
class ReferenceData{
  ...
  ReferenceData copyWith({
    ...
    ReferenceData(id:id,referenceNumber:referenceNumber??this.referenceNumber,
      createdAt:createdAt,updatedAt:updatedAt??this.updatedAt,
      scans:scans??this.scans);
  Map<String,dynamic>toMap()=>{'id':id,'referenceNumber':referenceNumber,
...
class PrintData{
  ...
  PrintData({required this.id,required this.name,required this.createdAt,
    required this.updatedAt,this.references=const[]});
  Map<String,dynamic>toMap()=>{'id':id,'name':name,'createdAt':createdAt,
    'updatedAt':updatedAt,'references':references.map((e)=>e.toMap()).toList()};
  factory PrintData.fromMap(Map<String,dynamic>m)=>PrintData(
    id:m['id']??Storage.id(),name:m['name']??'',
    createdAt:m['createdAt']??Storage.now(),updatedAt:m['updatedAt']??Storage.now(),
...
class TypeData{
  ...
  TypeData({required this.id,required this.name,required this.createdAt,
    required this.updatedAt,this.prints=const[]});
  Map<String,dynamic>toMap()=>{'id':id,'name':name,'createdAt':createdAt,
    'updatedAt':updatedAt,'prints':prints.map((e)=>e.toMap()).toList()};
...
class ModelData{
  ...
  ModelData({required this.id,required this.name,required this.createdAt,
    required this.updatedAt,this.types=const[]});
  Map<String,dynamic>toMap()=>{'id':id,'name':name,'createdAt':createdAt,
    'updatedAt':updatedAt,'types':types.map((e)=>e.toMap()).toList()};
...
class GroupData{
  ...
  GroupData({required this.id,required this.name,required this.temple,
    required this.createdAt,required this.updatedAt,this.models=const[]});
  Map<String,dynamic>toMap()=>{'id':id,'name':name,'temple':temple,
    'createdAt':createdAt,'updatedAt':updatedAt,
  ...
  factory GroupData.fromMap(Map<String,dynamic>m)=>GroupData(
    ...
    models:(m['models']as List???[]).map((e)=>
      ModelData.fromMap(Map<String,dynamic>.from(e))).toList());
}
...
class Storage{
  ...
  static Future<List<GroupData>>load()async{
    ...
    try{return(jsonDecode(raw)as List).map((e)=>
      GroupData.fromMap(Map<String,dynamic>.from(e))).toList();}
    catch(_){return[];}
  }
  ...
  static void renumber(PrintData p){
...
class _CreateGroupPageState extends State<CreateGroupPage>{
  ...
  Future<void>create()async{
    ...
    groups.add(GroupData(id:Storage.id(),name:name,temple:temple,
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    body:ListView(padding:const EdgeInsets.all(16),children:[
      TextField(controller:nameController,decoration:const InputDecoration(
        labelText:'ชื่อพระ *',hintText:'เช่น หลวงปู่ทวด',border:OutlineInputBorder())),
      const SizedBox(height:12),
      TextField(controller:templeController,decoration:const InputDecoration(
        labelText:'วัด / สำนัก *',hintText:'เช่น วัดช้างให้',border:OutlineInputBorder())),
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    ListView.builder(
      ...
      itemBuilder:(_,i){
        ...
        final refs=g.models.fold<int>(0,(s,m)=>s+m.types.fold<int>(
          0,(a,t)=>a+t.prints.fold<int>(0,(b,p)=>b+p.references.length)));
...
class _ModelListPageState extends State<ModelListPage>{
  ...
  Future<void>deleteModel(ModelData model)async{
    final refs=model.types.fold<int>(
      0,(s,t)=>s+t.prints.fold<int>(0,(a,p)=>a+p.references.length));
    final msg=model.types.isNotEmpty
    ...
    if(!await confirmDelete(context,'ลบรุ่น',msg))return;
    final gs=await Storage.load();
    for(final g in gs){
      if(g.id!=widget.groupId)continue;
      g.models.removeWhere((e)=>e.id==model.id);
      await Storage.save(gs);load();return;
    }
  }
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(title:Text(g?.name??'รายการ'),actions:[
        FilledButton.icon(onPressed:addModel,
          icon:const Icon(Icons.add,size:22),
          label:const Text('รุ่น',
      ...
      ListView.builder(
        ...
        itemBuilder:(_,i){
          final m=g.models[i];
          final refs=m.types.fold<int>(
            0,(s,t)=>s+t.prints.fold<int>(0,(a,p)=>a+p.references.length));
          return Card(child:ListTile(
            title:Text('รุ่น ${m.name}'),
            subtitle:Text('ชนิด ${m.types.length} รายการ\nอ้างอิง $refs องค์'),
            ...
            onTap:()=>Navigator.push(c,MaterialPageRoute(
              builder:(_)=>TypeListPage(
                cameras:widget.cameras,groupId:widget.groupId,modelId:m.id))),
          ));
        }));
  }
}
...
class CreateDataPage extends StatefulWidget{
  final List<CameraDescription>cameras;
  final String groupId;
  const CreateDataPage({required this.cameras,required this.groupId,super.key});
...
class _CreateDataPageState extends State<CreateDataPage>{
  ...
  Future<void>saveData()async{
    ...
    for(final g in groups){
      ...
      g.models.add(ModelData(id:Storage.id(),name:name,
        createdAt:now,updatedAt:now));
      await Storage.save(groups);
...
class TypeListPage extends StatefulWidget{
  ...
  const TypeListPage({
    required this.cameras,required this.groupId,required this.modelId,super.key});
...
class _TypeListPageState extends State<TypeListPage>{
  ...
  Future<void>addType()async{
          ...
          return AlertDialog(
            ...
            actions:[
              ...
              FilledButton(onPressed:(){
                ...
                if(name.isNotEmpty&&name!='อื่น ๆ')Navigator.pop(context,name);
    ...
    for(final g in gs){
      ...
      for(final m in g.models){
        ...
        final now=Storage.now();
        m.types.add(TypeData(id:Storage.id(),name:result,
          createdAt:now,updatedAt:now));
        await Storage.save(gs);load();return;
      }
    }
  ...
  Future<void>deleteType(TypeData type)async{
    final refs=type.prints.fold<int>(0,(s,p)=>s+p.references.length);
    final msg=type.prints.isNotEmpty
      ?'ชนิด "${type.name}" มี ${type.prints.length} พิมพ์ และองค์อ้างอิง $refs องค์\n'
       'การลบจะลบพิมพ์ องค์อ้างอิง และข้อมูลสแกนทั้งหมดด้วย\n\nต้องการลบใช่หรือไม่?'
      :'ต้องการลบชนิด "${type.name}" ใช่หรือไม่?';
    if(!await confirmDelete(context,'ลบชนิด',msg))return;
    final gs=await Storage.load();
    for(final g in gs){
      if(g.id!=widget.groupId)continue;
      for(final m in g.models){
  ...
  @override Widget build(BuildContext c){
    final m=model;
    return Scaffold(
      appBar:AppBar(title:Text('รุ่น ${m?.name??''}'),actions:[
        FilledButton.icon(onPressed:addType,
          icon:const Icon(Icons.add,size:22),
          label:const Text('ชนิด',
            style:TextStyle(fontSize:16,fontWeight:FontWeight.bold))),
      ]),
      ...
      ListView.builder(
        ...
        itemBuilder:(_,i){
          final t=m.types[i];
          final refs=t.prints.fold<int>(0,(s,p)=>s+p.references.length);
          return Card(child:ListTile(
            title:Text(t.name),
            subtitle:Text('พิมพ์ ${t.prints.length} รายการ\nอ้างอิง $refs องค์'),
            trailing:Row(mainAxisSize:MainAxisSize.min,children:[
              IconButton(icon:const Icon(Icons.remove_circle_outline),
                tooltip:'ลบชนิด',onPressed:()=>deleteType(t)),
              const Icon(Icons.chevron_right)]),
            onTap:()=>Navigator.push(c,MaterialPageRoute(
              builder:(_)=>PrintListPage(
                cameras:widget.cameras,groupId:widget.groupId,
                modelId:widget.modelId,typeId:t.id))),
          ));
        }));
  }
}
...
class PrintListPage extends StatefulWidget{
  ...
  @override State<PrintListPage>createState()=>_PrintListPageState();
}
...
class _PrintListPageState extends State<PrintListPage>{
    ...
    for(final g in gs){
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          if(t.id==widget.typeId&&mounted){
            setState(()=>type=t);return;
          }
        }
      }
    }
    ...
    final name=await showDialog<String>(
      context:context,builder:(_)=>AlertDialog(
        ...
        content:TextField(controller:c,autofocus:true,
          decoration:const InputDecoration(
            labelText:'ชื่อพิมพ์',hintText:'เช่น พิมพ์ใหญ่',
            border:OutlineInputBorder())),
        actions:[
          TextButton(onPressed:()=>Navigator.pop(context),
            child:const Text('ยกเลิก')),
    ...
    for(final g in gs){
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          ...
          final now=Storage.now();
          t.prints.add(PrintData(id:Storage.id(),name:name,
            createdAt:now,updatedAt:now));
          await Storage.save(gs);load();return;
        }
      }
  ...
  Future<void>deletePrint(PrintData p)async{
    final refs=p.references.length;
    final scans=p.references.fold<int>(0,(s,r)=>s+r.scans.length);
    final msg=refs>0
      ?'พิมพ์ "${p.name}" มีองค์อ้างอิง $refs องค์\n'
       'มีข้อมูลสแกน $scans ด้าน\n'
       'การลบจะลบองค์อ้างอิงและข้อมูลสแกนทั้งหมดด้วย\n\n'
       'ต้องการลบใช่หรือไม่?'
      :'ต้องการลบพิมพ์ "${p.name}" ใช่หรือไม่?';
    if(!await confirmDelete(context,'ลบพิมพ์',msg))return;
    final gs=await Storage.load();
    for(final g in gs){
      if(g.id!=widget.groupId)continue;
      for(final m in g.models){
        ...
        for(final t in m.types){
          if(t.id!=widget.typeId)continue;
          t.prints.removeWhere((e)=>e.id==p.id);
          await Storage.save(gs);load();return;
        }
      }
    }
  ...
  @override Widget build(BuildContext c){
    final t=type;
    return Scaffold(
      appBar:AppBar(title:Text(t?.name??'พิมพ์'),actions:[
        Padding(padding:const EdgeInsets.only(right:8),
          child:FilledButton.icon(onPressed:addPrint,
            icon:const Icon(Icons.add,size:24),
            label:const Text('พิมพ์',
              style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)))),
      ]),
      body:t==null?const Center(child:CircularProgressIndicator()):
      t.prints.isEmpty?const Center(child:Text(
        'ยังไม่มีพิมพ์\nกด + พิมพ์ เพื่อเพิ่ม',textAlign:TextAlign.center)):
      ListView.builder(
        padding:const EdgeInsets.all(12),itemCount:t.prints.length,
        itemBuilder:(_,i){
          final p=t.prints[i];
          final scans=p.references.fold<int>(0,(s,r)=>s+r.scans.length);
          return Card(child:ListTile(
            title:Text(p.name),
            subtitle:Text('อ้างอิง ${p.references.length} องค์\nข้อมูลสแกน $scans ด้าน'),
            trailing:Row(mainAxisSize:MainAxisSize.min,children:[
              IconButton(icon:const Icon(Icons.remove_circle_outline),
                tooltip:'ลบพิมพ์',onPressed:()=>deletePrint(p)),
              const Icon(Icons.chevron_right)]),
            onTap:()=>Navigator.push(c,MaterialPageRoute(
              builder:(_)=>ReferenceListPage(
                cameras:widget.cameras,groupId:widget.groupId,
                modelId:widget.modelId,typeId:widget.typeId,printId:p.id)))
              .then((_)=>load()),
          ));
        }));
  }
...
class ReferenceListPage extends StatefulWidget{
  final List<CameraDescription>cameras;
  final String groupId,modelId,typeId,printId;
  const ReferenceListPage({
    required this.cameras,required this.groupId,
    required this.modelId,required this.typeId,
    required this.printId,super.key});
...
class _ReferenceListPageState extends State<ReferenceListPage>{
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          ...
          for(final p in t.prints){
            if(p.id==widget.printId&&mounted){
              Storage.renumber(p);setState(()=>print=p);
              await Storage.save(gs);return;
            }
          }
        }
      }
  ...
  Future<void>addReference()async{
    final p=print;
    if(p==null)return;
    final ref=ReferenceData(id:Storage.id(),
      referenceNumber:p.references.length+1,
    ...
    for(final g in gs){
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          if(t.id!=widget.typeId)continue;
          for(final savedPrint in t.prints){
            if(savedPrint.id!=widget.printId)continue;
            savedPrint.references.add(ref);Storage.renumber(savedPrint);
            await Storage.save(gs);
            if(!mounted)return;
            await Navigator.push(context,MaterialPageRoute(
              builder:(_)=>ScanPage(
                cameras:widget.cameras,groupId:widget.groupId,
                modelId:widget.modelId,typeId:widget.typeId,
                printId:widget.printId,referenceId:ref.id)));
            load();return;
          }
        }
      }
    ...
    for(final g in gs){
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          if(t.id!=widget.typeId)continue;
          for(final p in t.prints){
            if(p.id!=widget.printId)continue;
            p.references.removeWhere((e)=>e.id==ref.id);
            Storage.renumber(p);await Storage.save(gs);load();return;
          }
        }
      }
    }
  ...
  @override Widget build(BuildContext c){
    final p=print;
    return Scaffold(
      appBar:AppBar(title:Text(p?.name??'อ้างอิง'),actions:[
        Padding(padding:const EdgeInsets.only(right:8),
          child:FilledButton.icon(onPressed:addReference,
            icon:const Icon(Icons.add,size:24),
            label:const Text('อ้างอิง',
              style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)))),
      ]),
      body:p==null?const Center(child:CircularProgressIndicator()):
      p.references.isEmpty?const Center(child:Text(
        'ยังไม่มีอ้างอิง\nกด + อ้างอิง เพื่อเพิ่มองค์อ้างอิง',
        textAlign:TextAlign.center)):
      ListView.builder(
        padding:const EdgeInsets.all(12),itemCount:p.references.length,
        itemBuilder:(_,i){
          final r=p.references[i];
          return Card(child:ListTile(
            leading:CircleAvatar(child:Text('${r.referenceNumber}')),
            title:Text('อ้างอิง ${r.referenceNumber}'),
            subtitle:Text('มีข้อมูล ${r.scans.length}/${areas.length} ด้าน'),
            trailing:PopupMenuButton<String>(
              onSelected:(v){if(v=='delete')deleteReference(r);},
              itemBuilder:(_)=>const[PopupMenuItem(
            ...
            onTap:()=>Navigator.push(c,MaterialPageRoute(
              builder:(_)=>ScanPage(
                cameras:widget.cameras,groupId:widget.groupId,
                modelId:widget.modelId,typeId:widget.typeId,
                printId:widget.printId,referenceId:r.id)))
              .then((_)=>load()),
          ));
        }));
  }
...
class ScanPage extends StatefulWidget{
  final List<CameraDescription>cameras;
  final String groupId,modelId,typeId,printId,referenceId;
  const ScanPage({
...
class _ScanPageState extends State<ScanPage>{
  ...
  @override void initState(){super.initState();loadReference();}
        ...
        for(final t in m.types){
          ...
          for(final p in t.prints){
            ...
            for(final r in p.references){
              if(r.id==widget.referenceId&&mounted){
                setState(()=>scans={for(final s in r.scans)s.area:s});return;
              }
            }
          }
        }
  ...
  Future<void>scanCamera()async{
    ...
    final file=await Navigator.push<XFile>(context,MaterialPageRoute(
      builder:(_)=>CameraScanPage(cameras:widget.cameras)));
    if(file==null)return;
    ...
    try{
      ...
      showMessage('ไม่สามารถบันทึกภาพจากกล้องได้');
      ...
      for(final m in g.models){
        ...
        for(final t in m.types){
          ...
          for(final p in t.prints){
            ...
            for(var i=0;i<p.references.length;i++){
              final r=p.references[i];
              if(r.id!=widget.referenceId)continue;
              final result=ScanResult(area:selectedArea,
                details:'รอระบบ AI วิเคราะห์สิ่งที่มองเห็นจากภาพ');
              p.references[i]=r.copyWith(updatedAt:Storage.now(),
                scans:[...r.scans.where((e)=>e.area!=selectedArea),result]);
              await Storage.save(gs);
              if(mounted){
        ...
        child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          ...
          Wrap(spacing:8,runSpacing:8,
            children:areas.map((a){
              final scanned=scans.containsKey(a);
              return ChoiceChip(
                label:Text(scanned?'$a ✓':a),selected:selectedArea==a,
                onSelected:(_)=>setState(()=>selectedArea=a));
            }).toList()),
        ]))),
      const SizedBox(height:12),
      Card(child:Padding(padding:const EdgeInsets.all(16),
        child:Column(children:[
          Text(scans.containsKey(selectedArea)
            ?'ด้านนี้มีข้อมูลแล้ว':'ด้านนี้ยังไม่ได้สแกน',
            style:const TextStyle(fontSize:17,fontWeight:FontWeight.bold)),
          const SizedBox(height:8),
          Text('เลือก $selectedArea แล้วใช้กล้อง หรือเลือกรูปจากแกลเลอรี',
            textAlign:TextAlign.center),
          const SizedBox(height:16),
          SizedBox(width:double.infinity,child:FilledButton.icon(
            onPressed:saving?null:scanCamera,icon:const Icon(Icons.camera_alt),
      ...
      Card(child:Padding(padding:const EdgeInsets.all(16),
        child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          ...
          ...areas.map((a)=>ListTile(
            dense:true,
            leading:Icon(scans.containsKey(a)
              ?Icons.check_circle:Icons.radio_button_unchecked),
            title:Text(a),
            subtitle:scans.containsKey(a)
              ?Text(scans[a]!.details):const Text('ยังไม่ได้สแกน'))),
        ]))),
      const SizedBox(height:12),
      const Card(child:Padding(padding:EdgeInsets.all(16),
        child:Text('รูปภาพใช้สำหรับการวิเคราะห์ชั่วคราว '
...
class _CameraScanPageState extends State<CameraScanPage>{
  ...
  Future<void>initCamera()async{
    try{
      ...
      final c=CameraController(camera,ResolutionPreset.medium,enableAudio:false);
      controller=c;initializeFuture=c.initialize();
  ...
  Future<void>takePhoto()async{
    ...
    try{
      ...
      if(mounted)ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content:Text('ถ่ายภาพไม่สำเร็จ')));
    }
  }
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      ...
      appBar:AppBar(backgroundColor:Colors.black,
        foregroundColor:Colors.white,title:const Text('ถ่ายภาพอ้างอิง')),
      body:error!=null
        ?Center(child:Text(error!,
            style:const TextStyle(color:Colors.white,fontSize:18)))
        :camera==null
          ?const Center(child:CircularProgressIndicator())
          :FutureBuilder<void>(future:initializeFuture,
              builder:(context,snapshot){
                if(snapshot.connectionState!=ConnectionState.done)
                  return const Center(child:CircularProgressIndicator());
                if(snapshot.hasError)
                  return const Center(child:Text('ไม่สามารถเปิดกล้องได้',
                ...
                return Stack(fit:StackFit.expand,children:[
                  ...
                  Positioned(bottom:30,left:0,right:0,
                    child:Center(child:FloatingActionButton(
                      heroTag:'take_photo',onPressed:takePhoto,
