...
int maxRef(List<AmuletData> items) {
  var max = 0;
  for (final e in items) {
    if (e.realItemNumber > max) max = e.realItemNumber;
  }
  return max;
}
...
Future<void> syncReferenceNumber(
  List<AmuletData> items,
) async {
  final p = await prefs();
  final current = p.getInt(nextNumberKey) ?? 1;
  final required = maxRef(items) + 1;
  if (required > current) {
    await p.setInt(nextNumberKey, required);
  }
}
...
Future<int> nextReferenceNumber(
  ...
  await syncReferenceNumber(items);
  final p = await prefs();
  return p.getInt(nextNumberKey) ?? 1;
}
...
Future<void> commitReferenceNumber(int number) async {
  ...
  final current = p.getInt(nextNumberKey) ?? 1;
  if (number + 1 > current) {
    await p.setInt(nextNumberKey, number + 1);
  }
}

Future<bool> repairReferenceNumbers(
  List<AmuletData> items,
) async {
  final p = await prefs();
  final stored = p.getInt(nextNumberKey) ?? 1;
  var next = stored > maxRef(items) + 1
      ? stored
      : maxRef(items) + 1;
  ...
  for (final item in items) {
    final n = item.realItemNumber;

    if (n <= 0 || used.contains(n)) {
      while (used.contains(next) || next <= 0) {
        next++;
      }
      item.realItemNumber = next++;
      changed = true;
    }

    used.add(item.realItemNumber);
  }
...
Future<List<AmuletData>> loadAllItems() async {
  ...
  final compressed = p.getString(dataKey);

  if (compressed != null && compressed.isNotEmpty) {
    try {
      final data = jsonDecode(
        utf8.decode(gzip.decode(base64Decode(compressed))),
      );
  ...
  if (items == null) {
    ...
    try {
      items = old
          .map((e) => AmuletData.fromMap(jsonDecode(e)))
          .toList();
  ...
  final repaired = await repairReferenceNumbers(items);

  if (repaired || compressed == null || compressed.isEmpty) {
...
Future<void> saveAllItems(List<AmuletData> items) async {
  ...
  final text = jsonEncode(items.map((e) => e.toMap()).toList());
  final bytes = gzip.encode(utf8.encode(text));

  await p.setString(dataKey, base64Encode(bytes));
  await p.remove(oldDataKey);
  await syncReferenceNumber(items);
}
...
class _CreateDataPageState extends State<CreateDataPage> {
  ...
  Future<void> save() async {
    try {
      ...
      if (mounted) {
        message(context, 'บันทึกข้อมูลไม่สำเร็จ: $e');
      }
    }
  }
...
class _SavedListPageState extends State<SavedListPage> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final g in groups.values) {
      g.sort((a, b) => a.realItemNumber.compareTo(b.realItemNumber));
    }
    ...
    filtered.sort(
      (a, b) => a.first.realItemNumber.compareTo(
        b.first.realItemNumber,
      ),
    );
...
class _EditDataPageState extends State<EditDataPage> {
  ...
  Future<void> save() async {
    try {
      ...
      if (i == -1) {
        if (mounted) {
          message(context, 'ไม่พบข้อมูลที่ต้องการแก้ไข');
        }
        return;
      }
      ...
      if (mounted) {
        message(context, 'แก้ไขข้อมูลไม่สำเร็จ: $e');
      }
    }
  }
...
class _AmuletGroupPageState extends State<AmuletGroupPage> {
  ...
  void initState() {
    ...
    group = [...widget.group]
      ..sort(
        (a, b) => a.realItemNumber.compareTo(b.realItemNumber),
      );
  }
  ...
  Future<void> refresh() async {
    ...
    final updated = all.where((e) => groupKey(e) == key).toList()
      ..sort(
        (a, b) => a.realItemNumber.compareTo(b.realItemNumber),
      );
  ...
  Future<void> deleteItem(AmuletData item) async {
    ...
    try {
      ...
      setState(() {
        group.removeWhere((e) => e.id == item.id);
      });
      ...
      if (mounted) {
        message(context, 'ลบข้อมูลไม่สำเร็จ: $e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final name =
        first.name.isEmpty ? 'ยังไม่ได้ระบุชื่อ' : first.name;
    final model =
        first.model.isEmpty ? 'ยังไม่ได้ระบุรุ่น' : first.model;
      ...
      body: ListView(
        ...
        children: [
          ...
          ...group.map((item) {
            const total = 6;
            final completed =
                item.scans.map((e) => e.area).toSet().length;
            final remaining = total - completed;

            return Card(
              child: ListTile(
                leading: CircleAvatar(
                  child: Text('${item.realItemNumber}'),
                ),
                title: Text(
                  'ลำดับอ้างอิงที่ ${item.realItemNumber}',
                ),
                    ...
                    IconButton(
                      ...
                      onPressed: () async {
                        final changed =
                          ...
                          context,
                          MaterialPageRoute(
                            builder: (_) => EditDataPage(item: item),
                          ),
                        );
...
class _BackupPageState extends State<BackupPage> {
  ...
  Future<void> backup() async {
    ...
    try {
      ...
      final next = p.getInt(nextNumberKey) ?? 1;

      final data = {
        'backupVersion': 3,
        'createdAt': DateTime.now().toIso8601String(),
        'containsImages': false,
        'nextRealItemNumber': next,
      ...
      if (mounted) {
        message(context, 'สำรองข้อมูลเรียบร้อยแล้ว');
      }
    } catch (e) {
      if (mounted) {
        message(context, 'สำรองข้อมูลไม่สำเร็จ: $e');
      }
    } finally {
  ...
  Future<void> importData() async {
    ...
    try {
      ...
      if (bytes == null) {
        throw Exception('ไม่สามารถอ่านไฟล์สำรองได้');
      }

      final decoded = jsonDecode(utf8.decode(bytes));

      if (decoded is! Map || decoded['data'] is! List) {
        throw Exception('รูปแบบไฟล์สำรองไม่ถูกต้อง');
      }
      ...
      await repairReferenceNumbers(items);
      ...
      final p = await prefs();
      final currentNext = p.getInt(nextNumberKey) ?? 1;

      final backupNext =
          decoded['nextRealItemNumber'] is int
              ? decoded['nextRealItemNumber'] as int
              : 1;

      final calculatedNext = maxRef(items) + 1;

      var finalNext = currentNext;
      if (backupNext > finalNext) finalNext = backupNext;
      if (calculatedNext > finalNext) finalNext = calculatedNext;

      await saveAllItems(items);
      await p.setInt(nextNumberKey, finalNext);
      ...
      if (mounted) {
        message(context, 'นำเข้าข้อมูลไม่สำเร็จ: $e');
      }
    } finally {
      if (mounted) setState(() => working = false);
    }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          SwitchListTile(
            title: const Text('เริ่มต้นด้วยสแกนองค์จริง'),
            subtitle: const Text('ตั้งวิธีสแกนเริ่มต้นเป็นกล้อง'),
            value: realObject,
            onChanged: (v) async {
          ...
          SwitchListTile(
            ...
            subtitle: const Text('แสดงรายการการสแกนที่ผ่านมา'),
            value: history,
            onChanged: (v) async {
              setState(() => history = v);
          ...
          SwitchListTile(
            title: const Text('เปลี่ยนพื้นที่ถัดไปอัตโนมัติ'),
            subtitle: const Text(
...
class _ScanPageState extends State<ScanPage> {
  ...
  void show(String text) {
    if (mounted) message(context, text);
  }
  ...
  List<String> areas() {
    ...
    for (final e in widget.item.scans) {
      if (!list.contains(e.area)) list.add(e.area);
    }
