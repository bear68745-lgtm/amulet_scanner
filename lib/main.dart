...
class _CreateReferencePageState
  ...
  Future<void> startScan() async {
    ...
    // =================================================
    // ถ้าหน้านี้ถูกเปิดมาจาก "เพิ่มองค์อ้างอิง"
    // เมื่อบันทึกสำเร็จ ให้ปิดหน้านี้ด้วย
    // เพื่อกลับไปหน้ารายการอ้างอิงทันที
    // =================================================
...
class _ReferenceListPageState
  ...
  Future<void> addNewReference() async {
    ...
    // ถ้าสร้างและบันทึกองค์ใหม่สำเร็จ
    // โหลดรายการใหม่ทันที
    if (saved == true) {
      await loadReferences();
    } else {
      // กรณีออกโดยไม่ได้บันทึก
      await loadReferences();
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'ชื่อพระ: ${reference.name}',
            ),
            Text(
              'รุ่น: ${reference.model}',
            ),
            Text(
              'ประเภท: ${reference.type}',
            ),
            Text(
              'พิมพ์: ${reference.pim}',
            ),
            Text(
              'วัด / สำนัก: ${reference.temple}',
            ),
          ],
        ),
      ),
  ...
  Future<void> saveReference() async {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        // สำคัญ:
        // ส่ง true กลับไปยังหน้าก่อนหน้า
        Navigator.pop(context, true);
      }
      ...
      // -----------------------------------------------
        ...
        // สำคัญ:
        // ส่ง true กลับไปยัง CreateReferencePage
        Navigator.pop(context, true);
      }
    } catch (_) {
      if (mounted) {
