...
const amuletObservationPoints={
  ...
  'พระเนื้อผง':[
    'พิมพ์ทรง','องค์พระ','ฐาน','รายละเอียดพิมพ์','เนื้อ','ผิว','คราบ','รอยตัดขอบ'
  ],

  'พระเนื้อดิน':[
    'พิมพ์ทรง','องค์พระ','ฐาน','รายละเอียดพิมพ์','เนื้อดิน','ผิว','คราบกรุ','รอยตัดขอบ'
  ],

  'นางพญา':[
    'พิมพ์ทรง','องค์พระ','ใบหน้า','ฐาน','รายละเอียดพิมพ์','เนื้อ','ผิว','รอยตัดขอบ'
  ],

  'ผงสุพรรณ':[
    'พิมพ์ทรง','องค์พระ','ใบหน้า','ฐาน','รายละเอียดพิมพ์','เนื้อ','ผิว','คราบกรุ','รอยตัดขอบ'
  ],

  'พระรอด':[
    'พิมพ์ทรง','องค์พระ','ซุ้ม','ฐาน','รายละเอียดพิมพ์','เนื้อ','ผิว','คราบกรุ'
  ],
  ...
  'พระขุนแผน':[
    'พิมพ์ทรง','องค์พระ','รายละเอียดใบหน้า','แขน/มือ','ฐาน','เนื้อ','ผิว','คราบกรุ'
  ],

  // ==========================================
  // หลวงปู่ทวดเนื้อหว้าน : จุดสังเกตปรับละเอียด
  // ==========================================
  ...
  'อื่น ๆ':[
    'พิมพ์ทรง','รายละเอียดองค์พระ','ลวดลาย','ขอบ','เนื้อ','ผิว','รอยตำหนิ','รายละเอียดเฉพาะรุ่น'
  ],
};
...
Map<String,dynamic> referenceModel()=> {
  ...
  'size':{'height':null,'width':null,'thickness':null,'weight':null,
    'range':[],'unit':'cm'},
...
class _CreateDataPageState extends State<CreateDataPage>{
  ...
  Future<void> save()async{
    ...
    items.add(AmuletData(
      id:newId(),name:nameController.text.trim(),model:modelController.text.trim(),
      pim:pimController.text.trim(),type:type,temple:templeController.text.trim(),
    ));
    await saveAllItems(items);
    if(!mounted)return;
    msg(context,'บันทึกข้อมูลเรียบร้อย');Navigator.pop(context);
  }
                ...
                children:gs.entries.map((g){
                  ...
                  return Card(
                    child:ListTile(
                      ...
                      onTap:()async{
                        await go(c,AmuletGroupPage(group:g.value,cameras:widget.cameras));
                        await load();
                      },
                    ),
                  );
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    body:ListView(
      ...
      children:[
        ...
        const SizedBox(height:12),
        DropdownButtonFormField<String>(
          value:amuletTypes.contains(type)?type:amuletTypes.first,
          decoration:const InputDecoration(labelText:'ชนิดพระ',border:OutlineInputBorder()),
          items:amuletTypes.map((e)=>DropdownMenuItem(value:e,child:Text(e))).toList(),
          onChanged:(v)=>setState(()=>type=v??type),
        ),
...
class _AmuletGroupPageState extends State<AmuletGroupPage>{
  ...
  Future<void> addReference()async{
    ...
    final e=AmuletData(
      id:newId(),name:a.name,model:a.model,pim:a.pim,type:a.type,temple:a.temple
    );
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    floatingActionButton:FloatingActionButton.extended(
      onPressed:addReference,icon:const Icon(Icons.add),label:const Text('เพิ่มรายการอ้างอิง'),
    ),
    body:ListView(
      padding:const EdgeInsets.all(12),
      children:[
...
class _ScanPageState extends State<ScanPage>{
  ...
  Future<void> openCamera()async{
    ...
    try{
      ...
      try{await camera?.stopImageStream();}catch(_){}
      await camera?.dispose();camera=null;scanning=false;
    }
  }
  ...
  Future<void> saveScan(String method)async{
    final s=ScanData(
      ...
      details:{
        ...
        'aiFindings':[],'aiNotes':'','aiConfidence':null,'aiAnalyzedAt':null,
        'sourceArea':area,'imageSaved':false,'temporaryOnly':true,
        'canCompareWithPreviousScans':true,'surfaceDetails':[],'defects':[],
        'moldDetails':[],'castingLines':[],'patternDetails':[],'observations':[],
        'referenceModel':{
        ...
        'referenceComparison':{
          'enabled':true,'referenceModelAvailable':false,'result':null,
          'differences':[],'sizeDifference':null,'proportionDifference':null,
        },
    ...
    if(autoNext){
      final next=areas().firstWhere((a)=>count(a)==0,orElse:()=>area);
      setState(()=>area=next);
    }else setState((){});
  }
  ...
  Future<void> addCustomArea()async{
    ...
    final ok=await showDialog<bool>(
      ...
      builder:(d)=>AlertDialog(
        ...
        content:TextField(controller:controller,decoration:const InputDecoration(labelText:'ชื่อพื้นที่')),
        actions:[
    ...
    final v=controller.text.trim();controller.dispose();
              ...
              child:Column(
                ...
                children:[
                  ...
                  ...list.map((a)=>RadioListTile<String>(
                    ...
                    onChanged:(v){if(v!=null)setState(()=>area=v);},
                  )),
                ],
              ),
            ),
              ...
              child:Column(
                ...
                children:[
                  ...
                  if(widget.item.scans.isEmpty)
                    const Padding(padding:EdgeInsets.only(top:8),child:Text('ยังไม่มีประวัติการสแกน'))
                  else ...widget.item.scans.map((s)=>ListTile(
                    dense:true,
                    leading:const Icon(Icons.check_circle_outline),
                    title:Text(s.area),subtitle:Text(s.method),
                    trailing:Text(s.details['aiStatus']=='pending'?'รอ AI':'วิเคราะห์แล้ว'),
                  )),
                ],
              ),
            ),
            ...
            child:Padding(
              ...
              child:Column(
                ...
                children:[
                  ...
                  const SizedBox(height:6),
                  const Text('AI สามารถสร้าง Reference Model จากข้อมูลอ้างอิงหลายแหล่งได้ โดยไม่ใช้ขนาดพิกเซลของรูปแทนขนาดจริง'),
                  const SizedBox(height:6),
                  const Text('ยังไม่ใช่การวัดองค์จริง และจะไม่ใช้ขนาดเพียงอย่างเดียวในการตัดสินความแท้'),
                ],
              ),
            ),
          ),
...
class _BackupPageState extends State<BackupPage>{
  Future<void> backup()async{
    ...
    final path=await FilePicker.platform.saveFile(
      dialogTitle:'บันทึกไฟล์สำรอง',fileName:'amulet_backup.json',
  ...
  Future<void> restore()async{
    ...
    try{
      ...
      if(ok!=true)return;
      await saveAllItems(list);
      if(mounted)msg(context,'นำเข้าข้อมูลเรียบร้อย');
    }catch(e){
      if(mounted)msg(context,'ไฟล์ไม่ถูกต้อง');
    }
  }
  ...
  Future<void> deleteAll()async{
    final ok=await showDialog<bool>(
      ...
      builder:(c)=>AlertDialog(
        ...
        content:const Text('ข้อมูลพระ รายการอ้างอิง และประวัติการสแกนทั้งหมดจะถูกลบ'),
        actions:[
          TextButton(onPressed:()=>Navigator.pop(c,false),child:const Text('ยกเลิก')),
    ...
    if(ok!=true)return;
    await saveAllItems([]);
    if(mounted)msg(context,'ลบข้อมูลทั้งหมดแล้ว');
  }
    ...
    body:ListView(
      ...
      children:[
        Card(child:ListTile(
          ...
          subtitle:const Text('สร้างไฟล์ JSON โดยไม่มีรูปภาพ'),
          onTap:backup,
        )),
        Card(child:ListTile(
          leading:const Icon(Icons.download),
          title:const Text('นำเข้าข้อมูล'),
          subtitle:const Text('นำข้อมูลจากไฟล์ JSON กลับเข้าเครื่อง'),
          onTap:restore,
        )),
        const SizedBox(height:12),
        Card(child:ListTile(
          ...
          title:const Text('ลบข้อมูลทั้งหมด'),
          subtitle:const Text('ลบรายการที่บันทึก และประวัติการสแกนทั้งหมด'),
          onTap:deleteAll,
        )),
      ],
    ),
...
class AiReconstructionPage extends StatelessWidget{
  ...
  @override Widget build(BuildContext c){
    final embedding=scan.imageFeatures['embedding'];
    final active=embedding is List&&embedding.isNotEmpty;

    return Scaffold(
      appBar:AppBar(title:const Text('AI Reconstruction')),
      body:ListView(
        padding:const EdgeInsets.all(16),
        children:[
          Card(
            child:Padding(
              ...
              child:Column(
                children:[
                  ...
                  SizedBox(
                    height:260,
                    child:CustomPaint(painter:AiReconstructionPainter(active:active)),
                  ),
                ],
              ),
            ),
            ...
            child:Padding(
              ...
              child:Column(
                ...
                children:[
                  ...
                  const SizedBox(height:10),
                  ...[
                    'รูปร่างและสัดส่วน','ขนาดจากข้อมูลอ้างอิง','พื้นผิว','เนื้อ',
                    'ตำหนิ','รายละเอียดพิมพ์','ยันต์ / อักขระ / ตัวหนังสือ / ตัวเลข',
                    'เส้นหล่อ / ร่องรอยการสร้าง','การเปรียบเทียบองค์อ้างอิง',
                    'ข้อสังเกต','ความมั่นใจ',
                  ].map((e)=>ListTile(
                    dense:true,
                    leading:const Icon(Icons.auto_awesome),
                    title:Text(e),
                    subtitle:Text(e=='ขนาดจากข้อมูลอ้างอิง'
                      ?'ยังไม่มีข้อมูลอ้างอิง':'รอระบบ AI วิเคราะห์'),
                  )),
                ],
              ),
            ),
          ...
          const Card(
            child:Padding(
              ...
              child:Text(
                'Reference Model จะสร้างจากข้อมูลอ้างอิงหลายแหล่ง '
                'โดยแยกขนาดจริงออกจากขนาดพิกเซลของภาพ '
                'และหากข้อมูลต่างกันจะเก็บเป็นช่วงหรือค่าหลายค่า '
                'แทนการเดาตัวเลขเอง\n\n'
                'ขนาดและสัดส่วนเป็นหลักฐานประกอบการเปรียบเทียบ '
                'ไม่ใช้ตัดสินความแท้เพียงอย่างเดียว',
              ),
            ),
          ),
          const Card(
            child:Padding(
              padding:EdgeInsets.all(14),
              child:Text(
                'หน้านี้เป็นโหมดเตรียมระบบ AI สำหรับ MediaPipe/LiteRT '
                'ยังไม่ใช่ผลวิเคราะห์จริง และภาพต้นฉบับไม่ได้ถูกบันทึก',
              ),
            ),
          ),
        ],
...
class AiReconstructionPainter extends CustomPainter{
  ...
  @override void paint(Canvas c,Size s){
    ...
    final r=RRect.fromRectAndRadius(
      Rect.fromCenter(
        ...
        width:s.width*.55,height:s.height*.75,
      ),
    ...
    c.drawOval(
      Rect.fromCenter(
        center:Offset(s.width/2,s.height*.38),
        width:s.width*.25,height:s.height*.25,
      ),p,
    );
  ...
  @override bool shouldRepaint(covariant AiReconstructionPainter old)=>
      old.active!=active;
}
