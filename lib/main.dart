...
class Storage {
  ...
  static void renumber(TypeData type) {
    for (var i = 0; i < type.references.length; i++) {
      type.references[i] =
          type.references[i].copyWith(referenceNumber: i + 1);
    }
  }
}
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            _button('รายการที่บันทึก ($groupCount รายการ)', Icons.folder,
                () async {
              await Navigator.push(
                context,
                MaterialPageRoute(
  ...
  Widget _button(String text, IconData icon, VoidCallback onTap) => Card(
        child: ListTile(
          leading: Icon(icon),
          title: Text(text),
...
class _GroupListPageState extends State<GroupListPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: groups.isEmpty
            ? const Center(child: Text('ยังไม่มีรายการที่บันทึก'))
            : ListView.builder(
                padding: const EdgeInsets.all(12),
                itemCount: groups.length,
                itemBuilder: (_, i) {
                  final group = groups[i];
                  final modelText = group.models.isEmpty
                      ? 'ยังไม่มีรุ่น'
                      : group.models.map((m) => m.name).join(', ');
                  final typeCount = group.models.fold<int>(
                    0,
                    (sum, m) => sum + m.types.length,
                  );
      ...
      body: g == null
          ...
          : g.models.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    final model = g.models[i];

                    return Card(
                      child: ListTile(
                        title: Text('รุ่น ${model.name}'),
                        subtitle: Text('${model.types.length} ชนิด'),
                        trailing: const Icon(Icons.chevron_right),
                        onTap: () {
                          Navigator.push(
                            context,
...
class _ReferenceListPageState extends State<ReferenceListPage> {
  ...
  Future<void> addReference() async {
    ...
    final ref = ReferenceData(
      id: Storage.id(),
      referenceNumber: t.references.length + 1,
  ...
  Future<void> deleteReference(ReferenceData reference) async {
    if (!await confirmDelete(
      context,
      'ลบอ้างอิง',
      'ต้องการลบอ้างอิง ${reference.referenceNumber} ใช่หรือไม่?',
    )) return;
    ...
    for (final g in groups) {
      ...
      for (final m in g.models) {
        ...
        for (final t in m.types) {
          ...
          t.references.removeWhere((e) => e.id == reference.id);
          Storage.renumber(t);
          await Storage.save(groups);
          load();
          return;
                    ...
                    return Card(
                      child: ListTile(
                        leading: CircleAvatar(
                          child: Text('${ref.referenceNumber}'),
                        ),
        ...
        for (final t in m.types) {
          ...
          for (final ref in t.references) {
            if (ref.id == widget.referenceId && mounted) {
              setState(() {
                scans = {for (final s in ref.scans) s.area: s};
              });
              return;
            }
          }
