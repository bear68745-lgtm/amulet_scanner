  ...
  Future<void> save() async {
    try {
      ...
      x[i] = AmuletData(
        ...
        material: o.material,
        size: o.size,
        frontDetail: o.frontDetail,
        sideDetail: o.sideDetail,
        backDetail: o.backDetail,
        scans: o.scans,
      );
...
class _AmuletGroupPageState extends State<AmuletGroupPage> {
  ...
  Future<void> deleteItem(AmuletData item) async {
    ...
    try {
      ...
      message(
        context,
        'ลบข้อมูลทั้งหมดของรายการอ้างอิงแล้ว',
      );
...
class _SettingsPageState
  ...
  Future<void> loadSettings() async {
    ...
    setState(() {
      realObject =
          p.getBool(keys[0]) ?? true;
      history =
          p.getBool(keys[1]) ?? true;
      autoNext =
          p.getBool(keys[2]) ?? true;
    });
  }
...
class _ScanPageState extends State<ScanPage> {
  ...
  Future<void> saveScan(String method) async {
    ...
    show(
      'บันทึกอัตโนมัติแล้ว • $old • สแกนครั้งที่ $n',
    );
  }
