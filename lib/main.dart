  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    ListView.builder(
      ...
      itemBuilder:(_,i){
        ...
        final refs=g.models.fold<int>(
          0,(s,m)=>s+m.types.fold<int>(
            0,(tCount,t)=>tCount+t.prints.fold<int>(
              0,(pCount,p)=>pCount+p.references.length)));
...
class _CreateDataPageState extends State<CreateDataPage>{
  ...
  @override void dispose(){
    modelController.dispose();super.dispose();
  }
  ...
  Future<void>saveData()async{
    ...
    if(name.isEmpty){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content:Text('กรุณากรอกชื่อรุ่น')));
      return;
    }
