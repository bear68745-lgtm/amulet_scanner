...
final amuletPatternPoints=<String,List>{
  'เหรียญ':[
...
List patternPointsFor(String type)=>
    amuletPatternPoints[type]??_commonClayPattern;

List detailPointsFor(String type){
  switch(type){
    case 'เหรียญ':
      return [
    ...
    case 'ผงสุพรรณ':
      return [..._commonDetail,'กระรอก','รอยนิ้วมือด้านหลัง'];
    case 'พระรอด':
      return [..._commonDetail,'รอยนิ้วมือด้านหลัง'];
    case 'อื่น ๆ':
...
List observationPointsFor(String type)=>[
  ...
  'flow':[
    {
      ...
      'name':'ด้านหน้า',
      'priority':'primary',
      'requirePreviousPass':false
    },
    {
      'stage':_aiBack,
      'name':'ด้านหลัง',
      'priority':'primary',
      'requirePreviousPass':true
    },
    {
      'stage':_aiSideBottom,
      'name':'ด้านข้าง / ก้นพระ',
      'priority':'primary',
      'requirePreviousPass':true
    },
    {
      'stage':_aiDetails,
      'name':'รายละเอียดประกอบ',
      'priority':'secondary',
      'requirePreviousPass':true
    }
  ],
...
Map<String,dynamic> toAiData(AmuletData e)=> {
  ...
  'referenceComparison':{
    ...
    'back':{
      'status':'locked_until_front_pass',
      'result':null,
      'matches':[],
      'differences':[]
    },
    'sideBottom':{
      'status':'locked_until_back_pass',
      'result':null,
      'matches':[],
      'differences':[]
    },
    'details':{
      'status':'locked_until_side_bottom_pass',
      'result':null,
      'matches':[],
      'differences':[],
      'notVisible':[]
...
class _ScanPageState extends State<ScanPage>{
  ...
  Future<void> openCamera()async{
    ...
    try{
      ...
      if(completed){
        ...
        msg(
          context,
          'ยกเลิกการสแกน ยังไม่มีการบันทึกข้อมูล'
        );
      }
    }catch(e){
      if(mounted){
   ...
   * รายละเอียด
   *
   * ด้านข้างและก้นพระเป็นขั้น AI เดียวกัน
   * จึงไม่บังคับว่าต้องสแกนทั้งสองด้าน
   */
  String nextAutoArea(){
    // 1. หลังจากด้านหน้า ต้องไปด้านหลัง
    if(area=='ด้านหน้า'){
      if(count('ด้านหลัง')==0){
        return 'ด้านหลัง';
      }
    ...
    // 2. หลังจากด้านหลัง ให้เข้าสู่ขั้น
    // ด้านข้าง / ก้นพระ
    if(area=='ด้านหลัง'){
      if(count('ด้านข้าง')==0){
        return 'ด้านข้าง';
    ...
    // 3. ถ้าสแกนด้านข้างแล้ว
    // และยังไม่เคยสแกนก้นพระ
    // สามารถให้ก้นพระเป็นพื้นที่เพิ่มเติมได้
    if(area=='ด้านข้าง'){
      ...
      details:{
        ...
        'authenticityDecision':{
          ...
          'note':
              ...
              'ด้านหน้า → ด้านหลัง → ด้านข้าง/ก้นพระ '
              '→ รายละเอียด '
              'โดยแยกความสึกจากการใช้งานออกจากความแตกต่าง '
              'ของพิมพ์ และไม่ตัดสินแท้/เก๊จากจุดใดจุดหนึ่ง '
              'เพียงอย่างเดียว',
        },
      },
    ...
    children:[
      ...
      ...list.map(
        (e)=>ListTile(
          ...
          title:Text('$e'),
          subtitle:e==_comparePoint
            ?const Text(
                'ใช้เป็นจุดสังเกตสำหรับเปรียบเทียบ '
  ...
  Widget build(BuildContext c){
    ...
    final pattern=
        patternPointsFor(widget.item.type);

    final details=
        detailPointsFor(widget.item.type);
            ...
            Card(
              child:ExpansionTile(
                ...
                children:widget.item.scans.isEmpty
                  ...
                  :widget.item.scans.reversed.map(
                      (s)=>ListTile(
                        leading:const Icon(
    ...
    body:ListView(
      children:[
        ...
        const Padding(
          padding:EdgeInsets.all(16),
          child:Text(
            'ระบบยังไม่ตัดสินแท้/เก๊จากการสแกน '
            'ข้อมูลการเปรียบเทียบจะถูกเตรียมไว้สำหรับระบบ AI '
            'ในขั้นต่อไป'
          ),
        ),
      ],
