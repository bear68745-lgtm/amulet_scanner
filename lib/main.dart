  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(title:Text(g?.name??'รายการ'),actions:[
        IconButton(onPressed:addModel,icon:const Icon(Icons.add),
          tooltip:'เพิ่มรุ่น'),
      ]),
  ...
  @override Widget build(BuildContext c){
    ...
    return Scaffold(
      appBar:AppBar(
        title:Text('รุ่น ${m?.name??''}'),
        actions:[
          IconButton(onPressed:addType,icon:const Icon(Icons.add),
            tooltip:'เพิ่มชนิด'),
        ]),
        ...
        for(final t in m.types){
          ...
          for(final ref in t.references){
            if(ref.id==widget.referenceId&&mounted){
              setState(()=>scans={for(final s in ref.scans)s.area:s});return;
            }
          }
        }
      }
        ...
        for(final t in m.types){
          ...
          for(var i=0;i<t.references.length;i++){
            ...
            final result=ScanResult(
              area:selectedArea,status:'มีข้อมูล',
            ...
            t.references[i]=ref.copyWith(
              ...
              scans:[...ref.scans.where((e)=>e.area!=selectedArea),result]);
            await Storage.save(groups);
            if(mounted)setState(()=>scans[selectedArea]=result);
            showMessage('บันทึกข้อมูล $selectedArea แล้ว');return;
          }
        }
      }
    }
    ...
    body:ListView(padding:const EdgeInsets.all(16),children:[
      Card(child:Padding(
        ...
        child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          ...
          const SizedBox(height:12),
          Wrap(spacing:8,runSpacing:8,children:areas.map((area){
