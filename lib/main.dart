...
class Storage {
  ...
  static Future<void> clearAiByScope({
    ...
    final matchedK = k.where((e) {
      final byRef = refs.contains(e.referenceId);
      final byGroup = groupId != null && e.groupId == groupId;
      final byModel = modelId != null && e.modelId == modelId;
      final byType = typeId != null && e.typeId == typeId;
      final byPrint = printId != null && e.printId == printId;

      return byRef || byGroup || byModel || byType || byPrint;
    }).toList();

    final kid = matchedK.map((e) => e.id).toSet();
    final ref = {
      ...refs,
      ...matchedK.map((e) => e.referenceId),
    };

    k.removeWhere((e) => kid.contains(e.id));
    h.removeWhere(
      (e) =>
          ref.contains(e.referenceId) ||
  ...
  static Future<void> rebuildAi(
    ...
    // สำคัญ:
    // ล้างความจำเดิมก่อนเสมอ
    // แม้ข้อมูลปัจจุบันจะไม่มี scan ก็ตาม
  ...
  static Future<void> syncScan({
    ...
    // ถ้าว่าง = ไม่ให้มีความจำเก่าเหลือ
  ...
  static Set<String> _refsFromType(TypeData t) => t.prints
      .expand((p) => p.references)
      .map((r) => r.id)
      .toSet();

  static Set<String> _refsFromModel(ModelData m) => m.types
      .expand((t) => t.prints)
      .expand((p) => p.references)
      .map((r) => r.id)
      .toSet();
...
class _ReferenceEditPageState
  ...
  Future<void> scanArea(String area) async {
    ...
    // 1. ฐานข้อมูลเปลี่ยนก่อน
    await Storage.saveGroup(widget.group);

    // 2. AI เปลี่ยนตามทันที
    //    เฉพาะองค์อ้างอิง + ด้านที่แก้
