...
class _ScanHomePageState extends State<ScanHomePage> {
  ...
  Future<void> _scanAmulet() async {
    ...
    try {
      // ถ่ายภาพเพื่อใช้วิเคราะห์ชั่วคราว
      // ไม่บันทึกภาพลงเครื่อง
      final XFile image = await controller!.takePicture();

      if (!mounted) return;

      // ตอนนี้เป็นขั้นทดสอบระบบสแกน
      // ภาพจะถูกนำไปใช้ต่อในระบบวิเคราะห์ภายหลัง
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('เริ่มสแกนแล้ว กำลังวิเคราะห์พระ...'),
          duration: Duration(seconds: 2),
        ),
      );

      // ไม่เก็บภาพไว้ในเครื่อง
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('ไม่สามารถเริ่มสแกนได้: $e'),
        ),
      );
    } finally {
      if (mounted) {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: isScanning ? null : _scanAmulet,
                    child: Text(
