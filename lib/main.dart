...
class Storage{
  ...
  static Future<void> saveHistory(List<AiLearningHistory> x)async{
    final p=await SharedPreferences.getInstance();
    await p.setString(
      historyKey,jsonEncode(x.map((e)=>e.toMap()).toList()));
  }
  ...
  static Future<void> saveTests(List<AiMemoryTest> x)async{
    final p=await SharedPreferences.getInstance();
    await p.setString(
      testKey,jsonEncode(x.map((e)=>e.toMap()).toList()));
  }

  static Future<void> addTest(AiMemoryTest x)async{
    final a=await loadTests();
    a.add(x);
    await saveTests(a);
  }
  ...
  static Future<void> saveResults(List<AiTestResult> x)async{
    final p=await SharedPreferences.getInstance();
    await p.setString(
      resultKey,jsonEncode(x.map((e)=>e.toMap()).toList()));
  }

  static Future<void> addResult(AiTestResult x)async{
    final a=await loadResults();
    a.add(x);
    await saveResults(a);
  }
...
class AiMemoryTestPage extends StatefulWidget{
  ...
  @override
  State<AiMemoryTestPage> createState()=>_AiMemoryTestPageState();
}
...
class _AiMemoryTestPageState extends State<AiMemoryTestPage>{
  ...
  Future<void> autoCheck()async{
    ...
    if(a.isEmpty){
      ...
      final score=rw.isEmpty
        ?0
        :aw.intersection(rw).length/rw.length;

      if(score>=.5){
        r='ใกล้เคียง';
        why='พบข้อมูลบางส่วนตรงกัน';
      }else{
        r='ผิด';
        why='ไม่พบข้อมูลตรงกันเพียงพอ';
      }
    }
  ...
  Future<void> manualCheck()async{
    ...
    if(r!=null){
      await saveResult('ผู้ใช้ตรวจ',r,'ผู้ใช้เป็นผู้ยืนยันผล');
    }
  }

  Future<void> saveResult(
    String m,String r,String why)async{
    ...
    await Storage.addResult(AiTestResult(
      ...
      result:r,
      answer:test!.answer,
      referenceAnswer:selected!.content,
      reason:why,
...
class ScanPage extends StatelessWidget{
  ...
  Future<void> scanCamera(BuildContext context)async{
    ...
    try{
      ...
      /* ลบไฟล์ภาพจากกล้องหลังใช้งานเสร็จ */
      try{
    ...
    /* AiVisionPage เป็นหน้าที่คืน ScanResult */
      ...
      child:Column(
        ...
        children:[
          const Text(
            ...
            'ระบบจะเก็บเฉพาะข้อมูลที่เลือกบันทึก',
            textAlign:TextAlign.center),
          const SizedBox(height:20),
          SizedBox(
            width:double.infinity,
...
class _AiVisionPageState extends State<AiVisionPage>{
  final fields={
    'พิมพ์ทรง':TextEditingController(),
    'องค์ประกอบ':TextEditingController(),
    'ลวดลาย':TextEditingController(),
    'ตำหนิที่มองเห็น':TextEditingController(),
    'ผิว':TextEditingController(),
    'ลักษณะเนื้อที่มองเห็น':TextEditingController(),
    'ขอบ / ด้านข้าง':TextEditingController(),
    'จุดสังเกต':TextEditingController(),
    'รายละเอียดอื่น':TextEditingController(),
    'สิ่งที่อ่านไม่ได้':TextEditingController(),
  };
  ...
  void save(){
    ...
    if(text.isEmpty){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:Text('กรุณาเลือกหัวข้อและใส่ข้อมูลก่อน')));
      return;
    }
  ...
  Widget build(BuildContext context)=>Scaffold(
    appBar:AppBar(
      title:Text('ข้อมูล AI ${widget.area}')),
    body:ListView(
      padding:const EdgeInsets.all(12),
      children:[
        ...
        const Text(
          ...
          'ระบบจะเก็บเฉพาะข้อความ ไม่เก็บรูปภาพ'),
        const SizedBox(height:12),
