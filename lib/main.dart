...
const amuletObservationPoints={
  ...
  'พระปิดตา':['รูปทรงองค์พระ','มือ/แขน','ใบหน้า','ฐาน','รายละเอียดพิมพ์','เนื้อ','ผิว','รอยตัด/รอยหล่อ'],
  'พระปิดตาเนื้อโลหะ':['รูปทรงองค์พระ','มือ/แขน','รายละเอียดพิมพ์','เส้นหล่อ','ผิวโลหะ','รอยตัด','ขอบ','ก้นพระ'],
  ...
  'นางพญ':['พิมพ์ทรง','องค์พระ','ใบหน้า','ฐาน','รายละเอียดพิมพ์','เนื้อ','ผิว','รอยตัดขอบ'],
...
class _CreateDataPageState extends State<CreateDataPage>{
  ...
  @override void dispose(){
    nameController.dispose();modelController.dispose();pimController.dispose();
    templeController.dispose();super.dispose();
  }
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    body:ListView(padding:const EdgeInsets.all(16),children:[
      TextField(controller:nameController,
        decoration:const InputDecoration(labelText:'ชื่อพระ',border:OutlineInputBorder())),
      const SizedBox(height:12),
      TextField(controller:modelController,
        decoration:const InputDecoration(labelText:'รุ่น',border:OutlineInputBorder())),
      const SizedBox(height:12),
      ...
      TextField(controller:pimController,
        decoration:const InputDecoration(labelText:'พิมพ์',border:OutlineInputBorder())),
      const SizedBox(height:12),
      TextField(controller:templeController,
        decoration:const InputDecoration(labelText:'วัด',border:OutlineInputBorder())),
...
class _AmuletGroupPageState extends State<AmuletGroupPage>{
  ...
  @override void initState(){
    super.initState();group=[...widget.group]..sort((a,b)=>a.id.compareTo(b.id));
  }
  ...
  Future<void> deleteItem(AmuletData item)async{
    final ok=await showDialog<bool>(
      context:context,builder:(c)=>AlertDialog(
        ...
        content:Text('ต้องการลบรายการองค์จริงนี้หรือไม่?'),
        actions:[
          TextButton(onPressed:()=>Navigator.pop(c,false),child:const Text('ยกเลิก')),
          FilledButton(onPressed:()=>Navigator.pop(c,true),child:const Text('ลบ')),
        ],
            ...
            content:SizedBox(
              ...
              child:Column(children:[
                Expanded(child:camera!.value.isInitialized
                  ?CameraPreview(camera!):const Center(
    ...
    return Scaffold(
      ...
      body:ListView(padding:const EdgeInsets.all(12),children:[
        ...
        Card(child:Padding(
          ...
          child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
            Row(children:[
              const Expanded(child:Text('พื้นที่สแกน',
                style:TextStyle(fontSize:18,fontWeight:FontWeight.bold))),
              TextButton.icon(
                onPressed:addCustomArea,
                icon:const Icon(Icons.add),label:const Text('เพิ่ม')),
            ]),
            ...list.map((a)=>RadioListTile<String>(
              value:a,groupValue:area,
              title:Text(a),
  ...
  Future<void> restore()async{
    ...
    try{
      ...
      final list=(data['items'] as List)
        .map((e)=>AmuletData.fromMap(Map<String,dynamic>.from(e))).toList();
      if(!mounted)return;
      final ok=await showDialog<bool>(
        context:context,builder:(c)=>AlertDialog(
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    body:ListView(padding:const EdgeInsets.all(12),children:[
      ...
      Card(child:ListTile(
        ...
        title:const Text('ลบข้อมูลทั้งหมด'),
        subtitle:const Text('ลบรายการที่บันทึกและประวัติการสแกนทั้งหมด'),
        onTap:deleteAll,
      )),
    ]),
  );
  ...
  @override Widget build(BuildContext c)=>Scaffold(
    ...
    body:ListView(children:[
      SwitchListTile(
        ...
        value:real,onChanged:(v){setState(()=>real=v);
          set('setting_real_object_default',v);}),
      SwitchListTile(
        title:const Text('แสดงประวัติการสแกน'),
        value:history,onChanged:(v){setState(()=>history=v);
          set('setting_show_scan_history',v);}),
      SwitchListTile(
        title:const Text('ไปพื้นที่ถัดไปอัตโนมัติ'),
        value:auto,onChanged:(v){setState(()=>auto=v);
          set('setting_auto_next_area',v);}),
      const Divider(),
      const ListTile(
        leading:Icon(Icons.info_outline),
        title:Text('พื้นฐาน 5+ องค์'),
        subtitle:Text(
          'เป็นเพียงระดับพื้นฐาน/ตัวเตือนความจำ ไม่จำกัดจำนวนองค์ '
          'สามารถเพิ่มข้อมูลได้เรื่อย ๆ'),
      ),
      const ListTile(
        leading:Icon(Icons.photo_camera_back_outlined),
        title:Text('นโยบายรูปภาพ'),
        subtitle:Text(
          'ภาพสแกนใช้ชั่วคราวเพื่อการประมวลผล และไม่บันทึกเป็นรูปภาพ'),
      ),
    ]),
  );
}
    ...
    body:ListView(padding:const EdgeInsets.all(16),children:[
      ...
      Card(child:Padding(
        ...
        child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          ...
          const SizedBox(height:10),
          ...[
            'รูปร่าง','พื้นผิว','ตำหนิ','เส้นหล่อ',
            'รายละเอียดพิมพ์','ข้อสังเกต','ความมั่นใจ'
          ].map((e)=>ListTile(
            dense:true,leading:const Icon(Icons.auto_awesome),
            title:Text(e),subtitle:const Text('รอระบบ AI วิเคราะห์'))),
        ]),
      )),
      Card(child:Padding(
        padding:const EdgeInsets.all(14),
...
class AiReconstructionPainter extends CustomPainter{
  ...
  @override void paint(Canvas c,Size s){
    ...
    final r=RRect.fromRectAndRadius(
      Rect.fromCenter(center:s.center,width:s.width*.55,height:s.height*.75),
    ...
    c.drawLine(
      ...
      Offset(s.width*.72,s.height*.62),p);
    c.drawLine(
      Offset(s.width*.35,s.height*.70),
      Offset(s.width*.65,s.height*.70),p);
    if(active){
