...
class Storage{
  ...
  static Future<void> save(List<GroupData> groups)async{
    final p=await SharedPreferences.getInstance();
    await p.setString(key,jsonEncode(groups.map((e)=>e.toMap()).toList()));
  }
  ...
  static Future<List<AiTestResult>> loadResults()async{
    final p=await SharedPreferences.getInstance();
    final raw=p.getString(resultKey);
    if(raw==null||raw.isEmpty)return[];
    try{
      return(jsonDecode(raw)as List)
  ...
  static Future<void> saveResults(List<AiTestResult> data)async{
    final p=await SharedPreferences.getInstance();
    await p.setString(
      resultKey,jsonEncode(data.map((e)=>e.toMap()).toList()));
  }

  static Future<void> addResult(AiTestResult r)async{
  ...
  static Future<void> addAi({
    ...
    final data=AiKnowledge(
      ...
      area:scan.area,content:scan.details,
      createdAt:i<0?now:list[i].createdAt,updatedAt:now);

    if(i<0)list.add(data);else list[i]=data;
    await saveAi(list);
    ...
    history.add(AiLearningHistory(
      ...
      content:scan.details,learnedAt:now));
...
class _AiMemoryTestPageState extends State<AiMemoryTestPage>{
  ...
  void selectKnowledge(AiKnowledge k){
    setState((){
      ...
      randomMode=false;
      answer.clear();
      test=null;result=null;mode=null;reason=null;
    });
  }

  void randomQuestion(){
    if(knowledge.isEmpty)return;
    final list=[...knowledge]..shuffle();
    final k=list.first;
    setState((){
      selected=k;
      question=makeQuestion(k);
  ...
  Future<void> saveAnswer()async{
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content:Text(
        'บันทึกคำตอบแล้ว เลือกวิธีตรวจคำตอบได้เลย')));
  }
  ...
  Future<void> autoCheck()async{
    ...
    if(a.isEmpty){
      ...
      r='ถูก';
      why='คำตอบตรงหรือมีเนื้อหาตรงกับความรู้ที่บันทึกไว้';
    }else{
      final aw=a.split(RegExp(r'[ ,./]+'))
        .where((e)=>e.isNotEmpty).toSet();
      final rw=ref.split(RegExp(r'[ ,./]+'))
        .where((e)=>e.isNotEmpty).toSet();
      final common=aw.intersection(rw).length;
      final score=rw.isEmpty?0:common/rw.length;

      if(score>=.5){
        r='ใกล้เคียง';
        why='พบข้อมูลบางส่วนตรงกับความรู้เดิม';
      }else{
        r='ผิด';
        why='ไม่พบข้อมูลที่ตรงกับความรู้เดิมเพียงพอ';
      }
    }
    ...
    body:knowledge.isEmpty
      ...
      :ListView(
        ...
        children:[
          ...
          const Text('เลือกคำถามเอง หรือสุ่มจากความรู้ที่ AI เคยเรียน'),
          ...
          if(selected!=null)...[
            ...
            Card(
              child:Padding(
                ...
                child:Column(
                  ...
                  children:[
                    Text(
                      randomMode?'🎲 คำถามสุ่ม':'📋 คำถามที่เลือก',
            ...
            TextField(
              ...
              decoration:const InputDecoration(
                labelText:'คำตอบ AI',
                hintText:'ใส่คำตอบที่ AI จำได้จากความรู้เดิม',
...
class _GroupPageState extends State<GroupPage>{
  ...
  Future<void> addModel()async{
    ...
    widget.group.models.add(ModelData(
      id:newId(),name:name.trim(),createdAt:DateTime.now()));
...
class _ModelPageState extends State<ModelPage>{
  ...
  Future<void> addType()async{
    ...
    widget.model.types.add(TypeData(
      id:newId(),name:type.trim(),createdAt:DateTime.now()));
    await save();
    setState((){});
  }
...
class _TypePageState extends State<TypePage>{
  ...
  Future<void> addPrint()async{
    ...
    widget.type.prints.add(PrintData(
      id:newId(),
      name:name.trim().isEmpty?'พิมพ์เดียว':name.trim(),
...
class _ReferencePageState extends State<ReferencePage>{
  ...
  Future<void> scan(String area)async{
    ...
    await Storage.addAi(
      ...
      print:widget.print,reference:widget.reference,scan:result);
    ...
    body:ListView(
      ...
      children:[
        ...
        const Text(
          ...
          'ขั้นที่ 3: ทดสอบความจำ AI\n'
          'ขั้นที่ 4: ตรวจสอบคำตอบ AI 2 แบบ\n'
...
class ScanPage extends StatelessWidget{
  ...
  Future<void> scanCamera(BuildContext context)async{
    ...
    Navigator.pop(context,ScanResult(
      area:area,details:'รอระบบ AI วิเคราะห์สิ่งที่มองเห็นจากภาพ'));
  }
    ...
    content:Column(
      ...
      children:[
        DropdownButtonFormField<String>(
          ...
          decoration:const InputDecoration(
            border:const OutlineInputBorder(),labelText:'ประเภท')),
        if(selected=='อื่น ๆ')...[
          const SizedBox(height:10),
          TextField(
            controller:other,
            decoration:const InputDecoration(
              border:const OutlineInputBorder(),labelText:'ระบุประเภท')),
        ],
      ],
    ),
    actions:[
