...
class ReferenceStorage {
  ...
  static List<ReferenceData> _toList(List data) {
    return data
        .map(
  ...
  // ---------------------------------------------------
  // UPDATE เฉพาะองค์อ้างอิงที่เลือก
  // ---------------------------------------------------
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> editGroup(
    ...
    // กดแก้ไขจากหน้ากลุ่ม
    // จะเข้าหน้ารายการองค์อ้างอิง
    // เพื่อเลือกองค์ที่ต้องการแก้ไข
    await open(
      ReferenceListPage(
...
class _EditReferencePageState
  ...
  // ---------------------------------------------------
  // ปุ่มเดียวสำหรับแก้ไข
  // ---------------------------------------------------

  Future<void> editAndScan() async {
    final ok = await saveBasicInfo();
...
class _ScanPageState
  ...
  // ===================================================
  // สำคัญมาก
  // ถ้าแก้ไของค์เดิม -> UPDATE องค์เดิม
  // ถ้าสร้างใหม่ -> CREATE องค์ใหม่
  // ===================================================

  Future<void> saveReference() async {
    if (saving || results.isEmpty) {
      return;
    }
    ...
    try {
      ...
      if (widget.existingReference !=
        ...
        ScaffoldMessenger.of(
          ...
          SnackBar(
            content: Text(
              'อัปเดตองค์อ้างอิงที่ '
              '${old.referenceNumber} แล้ว',
            ),
          ),
        );
