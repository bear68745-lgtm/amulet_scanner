...
class Storage {
  ...
  static Future<void> saveGroups(List<GroupData> list) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      groupsKey,
  ...
  /* สำคัญ: บันทึกเฉพาะกลุ่มที่กำลังแก้
     ป้องกันข้อมูล nested หาย */
  static Future<void> saveGroup(GroupData group) async {
    final list = await groups();
    final i = list.indexWhere((e) => e.id == group.id);
  ...
  static Future<void> saveKnowledge(List<AiKnowledge> list) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      knowledgeKey,
      jsonEncode(list.map((e) => e.toMap()).toList()),
  ...
  static Future<void> saveHistory(List<AiLearningHistory> list) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      historyKey,
      jsonEncode(list.map((e) => e.toMap()).toList()),
  ...
  static Future<void> saveTests(List<AiMemoryTest> list) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      testsKey,
      jsonEncode(list.map((e) => e.toMap()).toList()),
  ...
  static Future<void> saveResults(List<AiTestResult> list) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      resultsKey,
      jsonEncode(list.map((e) => e.toMap()).toList()),
  ...
  static Future<void> deleteAiByGroup(String groupId) async {
    ...
    final groupKnowledgeIds =
        k.where((e) => e.groupId == groupId).map((e) => e.id).toSet();
  ...
  static Future<void> deleteAiByReference(String referenceId) async {
    final k = await knowledge();
    final h = await history();
    final t = await tests();
    final r = await results();
    ...
    final ids =
        k.where((e) => e.referenceId == referenceId).map((e) => e.id).toSet();

    await saveKnowledge(
      k.where((e) => e.referenceId != referenceId).toList(),
    );

    await saveHistory(
      h.where((e) => e.referenceId != referenceId).toList(),
    );
  ...
  static Future<void> deleteAiByPrint(String printId) async {
    final gs = await groups();
    final ids = <String>{};
  ...
  static Future<void> deleteAiByType(String typeId) async {
    final gs = await groups();
    final ids = <String>{};
  ...
  static Future<void> deleteAiByModel(String modelId) async {
    final gs = await groups();
    final ids = <String>{};
...
class _ModelPageState extends State<ModelPage> {
  Future<void> add() async {
    await textDialog(context, 'สร้างรุ่น', '', (v) async {
      final now = DateTime.now().toIso8601String();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          PathBar([widget.group.name, 'สร้างรุ่น']),
          Expanded(
...
class _TypePageState extends State<TypePage> {
  ...
  Future<void> edit(TypeData t) async {
    await textDialog(context, 'แก้ไขชนิด', t.name, (v) async {
      t.name = v;
      await Storage.saveGroup(widget.group);
      setState(() {});
    });
...
class _PrintPageState extends State<PrintPage> {
  Future<void> add() async {
    await textDialog(context, 'สร้างพิมพ์', '', (v) async {
      final now = DateTime.now().toIso8601String();
...
class _ReferencePageState extends State<ReferencePage> {
  Future<void> addReference() async {
    final maxNumber = widget.print.references.isEmpty
        ? 0
  ...
  Future<void> deleteReference(ReferenceData ref) async {
    ...
    await Storage.deleteAiByReference(ref.id);
  ...
  Future<void> editReference(ReferenceData ref) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('องค์ตัวอย่าง')),
      body: Column(
        children: [
          PathBar([
            widget.group.name,
                  ...
                  child: ExpansionTile(
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          tooltip: 'แก้ไข',
                          onPressed: () => editReference(ref),
                          icon: const Icon(Icons.edit),
                        ),
                        IconButton(
                          tooltip: 'ลบ',
                          onPressed: () => deleteReference(ref),
                          icon: const Icon(Icons.delete),
                        ),
                      ],
                    ),
                    children: [
                      ...areas.map(
                        (area) {
                          final scan = ref.scans.cast<ScanResult?>().firstWhere(
                                (e) => e?.area == area,
                                orElse: () => null,
                              );
                          ...
                          return ListTile(
                            title: Text(area),
                            subtitle: Text(
                              scan == null || scan.details.isEmpty
                                  ? 'ยังไม่มีข้อมูล'
                                  : 'มีข้อมูลแล้ว',
                            ),
...
class _ReferenceEditPageState extends State<ReferenceEditPage> {
  ...
  Future<void> scanArea(String area) async {
    ...
    widget.reference.updatedAt = DateTime.now().toIso8601String();
...
class ScanPage extends StatelessWidget {
  ...
  Future<void> camera(BuildContext context) async {
...
class _AiVisionPageState extends State<AiVisionPage> {
  ...
  void initState() {
    ...
    final old = widget.reference.scans.where(
