...
class Storage {
  ...
  // ===================================================
  //
  // ใช้หาเลของค์อ้างอิงจริงจาก referenceId
  //
  // AI Memory เก็บ referenceId
  // ส่วนผู้ใช้เห็น referenceNumber
  // ===================================================

  static Future<int> findReferenceNumber({
    required String groupId,
    required String referenceId,
  }) async {
    final list = await groups();
  ...
  static Future<List<AiTestResult>>
    ...
    // -------------------------------------------------
    // โหลดข้อมูลกลุ่มครั้งเดียว
    // เพื่อหาเลของค์อ้างอิงของ Memory
    // -------------------------------------------------
    ...
    for (final memory in memories) {
      ...
      // ------------------------------------------------
      // หาเลของค์อ้างอิงของ Memory
      // ------------------------------------------------

      int memoryReferenceNumber = 0;
          ...
          for (final type in model.types) {
            for (final print in type.prints) {
              for (final reference
                ...
                if (reference.id ==
                    memory.referenceId) {
                  memoryReferenceNumber =
                      reference.referenceNumber;
                  break;
                }
              }

              if (memoryReferenceNumber != 0) {
                break;
              }
            }
      ...
      await saveAiMemoryTest(
        AiMemoryTest(
          ...
          question:
              ...
              'กับข้อมูลอ้างอิง '
              '${memoryReferenceNumber > 0 ? '#$memoryReferenceNumber' : memory.referenceId}',
          answer: current,
          testedAt: time,
        ),
      );
