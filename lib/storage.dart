...
class Storage {
  ...
  static Future<int> nextReferenceNumber() async {
    final prefs = await SharedPreferences.getInstance();
    final next = (prefs.getInt(counterKey) ?? 0) + 1;
  ...
  static Future<bool> saveReference({
    ...
    final list = await groups();
    final group = _findGroup(list, groupId);
    if (group == null) return false;

    final model = _findModel(group, modelId);
    if (model == null) return false;

    final type = _findType(model, typeId);
    if (type == null) return false;

    final print = _findPrint(type, printId);
  ...
  static Future<bool> learnFromScan({
    ...
    if (!await saveAiKnowledge(knowledge)) {
      return false;
    }

    final changed =
        oldIndex < 0 ||
        existing[oldIndex].content.trim() != clean;
  ...
  static Future<List<AiTestResult>> compareAiMemory({
    required String groupId,
    required String modelId,
    required String typeId,
    required String printId,
    ...
    final memories = await findComparableAiMemory(
      groupId: groupId,
      modelId: modelId,
      typeId: typeId,
      printId: printId,
    ...
    final results = <AiTestResult>[];

    for (final memory in memories) {
      final testId = newId();
      final time = now();

      final comparison = _compareText(
        current,
        memory.content,
      );
      ...
      await saveAiMemoryTest(
        AiMemoryTest(
          ...
          question:
              ...
              'กับข้อมูลอ้างอิง ${memory.referenceId}',
          answer: current,
          testedAt: time,
        ),
      );
      ...
      final result = AiTestResult(
        ...
        answer: current,
        referenceAnswer: memory.content,
        reason: comparison['reason'] ?? '',
        checkedAt: time,
      );

      await saveAiTestResult(result);
      results.add(result);
    }
  ...
  static Map<String, String> _compareText(
    ...
    if (a.isEmpty || b.isEmpty) {
      return {
        'result': 'ไม่เพียงพอ',
        'reason': 'ข้อมูลที่ใช้เปรียบเทียบไม่เพียงพอ',
      };
    }
    ...
    if (all.isEmpty) {
      return {
        ...
        'reason': 'ไม่พบข้อมูลร่วมกัน',
      };
    }

    final score = common.length / all.length;
    final percent = (score * 100).round();

    String result;

    if (score >= 0.70) {
      result = 'ข้อมูลใกล้เคียงกัน';
    } else if (score >= 0.40) {
      result = 'มีข้อมูลบางส่วนตรงกัน';
    } else {
      result = 'ข้อมูลแตกต่างกัน';
    }

    return {
      'result': result,
      'reason': 'พบข้อมูลร่วมกันประมาณ $percent%',
    };
  }
