                    ...
                    return Card(
                      child:ListTile(
                        ...
                        onTap:()=>Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:(_)=>GroupPage(
                              g,groups,widget.cameras))))); // แก้แล้ว
                  }))
        ])));
}
...
class _AiMemoryTestPageState extends State<AiMemoryTestPage>{
  ...
  Future<void> correctKnowledge()async{
    ...
    if(mounted)ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content:Text('แก้ไขความรู้แล้ว และบันทึกการเรียนรู้ใหม่เรียบร้อย')));
  }
...
class ScanPage extends StatelessWidget{
  ...
  Future<void> enterKnowledge(BuildContext context)async{
    ...
    Navigator.pop(
      context,
      ScanResult(area:area,details:text.trim()));
  }
