...
class _AmuletGroupPageState
  ...
  @override
  Widget build(BuildContext context) {
    // ไม่ปล่อยให้หน้าขาวหลังกลับจากการสแกน
    // กรณีไม่มีข้อมูลจริง ให้ย้อนกลับแทน
    if (group.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text(
              ...
              return Card(
                child: ListTile(
                  ...
                  onTap: () async {
                    ...
                    // =================================================
                    // สำคัญ:
                    // ไม่กรอง group ใหม่หลังกลับจาก ScanPage
                    //
                    // ScanPage แก้ข้อมูลของ item ตัวเดิมโดยตรง
                    // และบันทึกลง SharedPreferences แล้ว
                    //
                    // ถ้า reload + filter ตรงนี้ อาจทำให้ group
                    // กลายเป็น [] และขึ้น "ไม่พบข้อมูล"
                    //
                    // ดังนั้นใช้รายการเดิม แล้วสั่ง rebuild
                    // =================================================
                ...
                if (saved == true) {
                  ...
                  final currentKeys =
                      group
                          .map(
                            (item) =>
                                '${item.name.trim().toLowerCase()}|||${item.model.trim().toLowerCase()}',
                          )
                          .toSet();
