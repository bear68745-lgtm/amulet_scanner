...
const scanAreas = [
  'ด้านหน้า','ด้านหลัง','ด้านข้าง','หูเหรียญ','ตูดพระ','จุดเฉพาะ',
];
...
void go(BuildContext c, Widget p) =>
...
class _EditGroupPageState extends State<EditGroupPage> {
  ...
  Future<void> save() async {
    try {
      ...
      msg(
        context,
        'แก้ไขข้อมูลหลักและรายการอ้างอิง ${widget.group.length} รายการแล้ว',
      );
      Navigator.pop(context, true);
    } catch (e) {
...
class _AmuletGroupPageState extends State<AmuletGroupPage> {
  ...
  Future<void> refresh() async {
    ...
    final x = await loadAllItems();
    final updated = x.where((e) => ids.contains(e.id)).toList()
  ...
  Future<void> addReference() async {
    ...
    try {
      ...
      await go(
        context,
        ScanPage(item: item, cameras: widget.cameras),
      );
      await refresh();
    } catch (e) {
